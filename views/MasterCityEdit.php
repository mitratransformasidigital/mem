...
var fmaster_cityedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fmaster_cityedit = currentForm = new ew.Form("fmaster_cityedit", "edit");

    // Add fields
    var fields = ew.vars.tables.master_city.fields;
    fmaster_cityedit.addFields([
    ...
    // Validate form
    fmaster_cityedit.validate = function () {
    ...
    // Form_CustomValidate
    fmaster_cityedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmaster_cityedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fmaster_cityedit.lists.province_id = <?= $Page->province_id->toClientList($Page) ?>;
    loadjs.done("fmaster_cityedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="fmaster_cityedit" id="fmaster_cityedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php
    if (in_array("master_office", explode(",", $Page->getCurrentDetailTable())) && $master_office->DetailEdit) {
?>
