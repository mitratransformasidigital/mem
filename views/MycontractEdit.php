...
loadjs.ready("head", function () {
    ...
    fmycontractedit = currentForm = new ew.Form("fmycontractedit", "edit");

    // Add fields
    var fields = ew.vars.tables.mycontract.fields;
    fmycontractedit.addFields([
    ...
    // Validate form
    fmycontractedit.validate = function () {
    ...
    // Form_CustomValidate
    fmycontractedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmycontractedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fmycontractedit.lists.office_id = <?= $Page->office_id->toClientList($Page) ?>;
    loadjs.done("fmycontractedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="fmycontractedit" id="fmycontractedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->contract_id->Visible) { // contract_id ?>
    <div id="r_contract_id" class="form-group row">
        <label id="elh_mycontract_contract_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->contract_id->caption() ?><?= $Page->contract_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->contract_id->cellAttributes() ?>>
<span id="el_mycontract_contract_id">
...
<input type="hidden" data-table="mycontract" data-field="x_contract_id" data-hidden="1" name="x_contract_id" id="x_contract_id" value="<?= HtmlEncode($Page->contract_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
<?php if ($Page->salary->Visible) { // salary ?>
    <div id="r_salary" class="form-group row">
        <label id="elh_mycontract_salary" for="x_salary" class="<?= $Page->LeftColumnClass ?>"><?= $Page->salary->caption() ?><?= $Page->salary->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php if ($Page->bonus->Visible) { // bonus ?>
    <div id="r_bonus" class="form-group row">
        <label id="elh_mycontract_bonus" for="x_bonus" class="<?= $Page->LeftColumnClass ?>"><?= $Page->bonus->caption() ?><?= $Page->bonus->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php if ($Page->thr->Visible) { // thr ?>
    <div id="r_thr" class="form-group row">
        <label id="elh_mycontract_thr" class="<?= $Page->LeftColumnClass ?>"><?= $Page->thr->caption() ?><?= $Page->thr->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->thr->cellAttributes() ?>>
<span id="el_mycontract_thr">
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->thr->isInvalidClass() ?>" data-table="mycontract" data-field="x_thr" name="x_thr[]" id="x_thr_454331" value="1"<?= ConvertToBool($Page->thr->CurrentValue) ? " checked" : "" ?><?= $Page->thr->editAttributes() ?> aria-describedby="x_thr_help">
    <label class="custom-control-label" for="x_thr_454331"></label>
</div>
...
<?php if ($Page->contract_start->Visible) { // contract_start ?>
    <div id="r_contract_start" class="form-group row">
        <label id="elh_mycontract_contract_start" for="x_contract_start" class="<?= $Page->LeftColumnClass ?>"><?= $Page->contract_start->caption() ?><?= $Page->contract_start->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<script>
loadjs.ready(["fmycontractedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fmycontractedit", "x_contract_start", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->contract_end->Visible) { // contract_end ?>
    <div id="r_contract_end" class="form-group row">
        <label id="elh_mycontract_contract_end" for="x_contract_end" class="<?= $Page->LeftColumnClass ?>"><?= $Page->contract_end->caption() ?><?= $Page->contract_end->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<script>
loadjs.ready(["fmycontractedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fmycontractedit", "x_contract_end", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->office_id->Visible) { // office_id ?>
    <div id="r_office_id" class="form-group row">
        <label id="elh_mycontract_office_id" for="x_office_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->office_id->caption() ?><?= $Page->office_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php if ($Page->notes->Visible) { // notes ?>
    <div id="r_notes" class="form-group row">
        <label id="elh_mycontract_notes" for="x_notes" class="<?= $Page->LeftColumnClass ?>"><?= $Page->notes->caption() ?><?= $Page->notes->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php if ($Page->contract_document->Visible) { // contract_document ?>
    <div id="r_contract_document" class="form-group row">
        <label id="elh_mycontract_contract_document" class="<?= $Page->LeftColumnClass ?>"><?= $Page->contract_document->caption() ?><?= $Page->contract_document->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php } else { ?>
<span id="el_mycontract_employee_username">
<input type="hidden" data-table="mycontract" data-field="x_employee_username" data-hidden="1" name="x_employee_username" id="x_employee_username" value="<?= HtmlEncode($Page->employee_username->CurrentValue) ?>">
</span>
<?php } ?>
<?php if (!$Page->IsModal) { ?>
