...
// Page object
$MasterHolidayEdit = &$Page;
?>
<script>
if (!ew.vars.tables.master_holiday) ew.vars.tables.master_holiday = <?= JsonEncode(GetClientVar("tables", "master_holiday")) ?>;
var currentForm, currentPageID;
var fmaster_holidayedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fmaster_holidayedit = currentForm = new ew.Form("fmaster_holidayedit", "edit");

    // Add fields
    var fields = ew.vars.tables.master_holiday.fields;
    fmaster_holidayedit.addFields([
        ["holiday_id", [fields.holiday_id.required ? ew.Validators.required(fields.holiday_id.caption) : null], fields.holiday_id.isInvalid],
    ...
    // Validate form
    fmaster_holidayedit.validate = function () {
    ...
    // Form_CustomValidate
    fmaster_holidayedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmaster_holidayedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fmaster_holidayedit.lists.shift_id = <?= $Page->shift_id->toClientList($Page) ?>;
    loadjs.done("fmaster_holidayedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="fmaster_holidayedit" id="fmaster_holidayedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<script>
loadjs.ready(["fmaster_holidayedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fmaster_holidayedit", "x_holiday_date", {"ignoreReadonly":true,"useCurrent":false,"format":5});
});
</script>
<?php } ?>
</span>
...
<input type="hidden" data-table="master_holiday" data-field="x_holiday_id" data-hidden="1" name="x_holiday_id" id="x_holiday_id" value="<?= HtmlEncode($Page->holiday_id->CurrentValue) ?>">
</span>
<?php if (!$Page->IsModal) { ?>
