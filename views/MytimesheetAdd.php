...
loadjs.ready("head", function () {
    ...
    fmytimesheetadd = currentForm = new ew.Form("fmytimesheetadd", "add");

    // Add fields
    var fields = ew.vars.tables.mytimesheet.fields;
    fmytimesheetadd.addFields([
    ...
    // Validate form
    fmytimesheetadd.validate = function () {
    ...
    // Form_CustomValidate
    fmytimesheetadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmytimesheetadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fmytimesheetadd.lists.year = <?= $Page->year->toClientList($Page) ?>;
    fmytimesheetadd.lists.month = <?= $Page->month->toClientList($Page) ?>;
    fmytimesheetadd.lists.approved = <?= $Page->approved->toClientList($Page) ?>;
    loadjs.done("fmytimesheetadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fmytimesheetadd" id="fmytimesheetadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<div class="ew-add-div"><!-- page* -->
    ...
    <?php } else { ?>
    <span id="el_mytimesheet_employee_username">
    <input type="hidden" data-table="mytimesheet" data-field="x_employee_username" data-hidden="1" name="x_employee_username" id="x_employee_username" value="<?= HtmlEncode($Page->employee_username->CurrentValue) ?>">
    </span>
    <?php } ?>
