...
// Page object
$EmployeeShiftEdit = &$Page;
?>
<script>
if (!ew.vars.tables.employee_shift) ew.vars.tables.employee_shift = <?= JsonEncode(GetClientVar("tables", "employee_shift")) ?>;
var currentForm, currentPageID;
var femployee_shiftedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    femployee_shiftedit = currentForm = new ew.Form("femployee_shiftedit", "edit");

    // Add fields
    var fields = ew.vars.tables.employee_shift.fields;
    femployee_shiftedit.addFields([
        ["es_id", [fields.es_id.required ? ew.Validators.required(fields.es_id.caption) : null], fields.es_id.isInvalid],
    ...
    // Validate form
    femployee_shiftedit.validate = function () {
    ...
    // Form_CustomValidate
    femployee_shiftedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    femployee_shiftedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    femployee_shiftedit.lists.employee_username = <?= $Page->employee_username->toClientList($Page) ?>;
    loadjs.done("femployee_shiftedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="femployee_shiftedit" id="femployee_shiftedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->shift_id->getDisplayValue($Page->shift_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="employee_shift" data-field="x_shift_id" data-hidden="1" name="x_shift_id" id="x_shift_id" value="<?= HtmlEncode($Page->shift_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["femployee_shiftedit", "datetimepicker"], function() {
    ew.createDateTimePicker("femployee_shiftedit", "x_start_date", {"ignoreReadonly":true,"useCurrent":false,"format":5});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["femployee_shiftedit", "datetimepicker"], function() {
    ew.createDateTimePicker("femployee_shiftedit", "x_end_date", {"ignoreReadonly":true,"useCurrent":false,"format":5});
});
</script>
<?php } ?>
</span>
...
<span id="el_employee_shift_es_id">
<input type="hidden" data-table="employee_shift" data-field="x_es_id" data-hidden="1" name="x_es_id" id="x_es_id" value="<?= HtmlEncode($Page->es_id->CurrentValue) ?>">
</span>
<?php if (!$Page->IsModal) { ?>
