...
// Page object
$EmployeePreview = &$Page;
?>
<?php $Page->showPageHeader(); ?>
<?php if ($Page->TotalRecords > 0) { ?>
<div class="card ew-grid employee"><!-- .card -->
...
<?php if ($Page->user_level->Visible) { // user_level ?>
    <?php if ($Page->SortUrl($Page->user_level) == "") { ?>
        <th class="<?= $Page->user_level->headerCellClass() ?>"><?= $Page->user_level->caption() ?></th>
    <?php } else { ?>
        <th class="<?= $Page->user_level->headerCellClass() ?>"><div class="ew-pointer" data-sort="<?= HtmlEncode($Page->user_level->Name) ?>" data-sort-order="<?= $Page->SortField == $Page->user_level->Name && $Page->SortOrder == "ASC" ? "DESC" : "ASC" ?>">
            <div class="ew-table-header-btn"><span class="ew-table-header-caption"><?= $Page->user_level->caption() ?></span><span class="ew-table-header-sort"><?php if ($Page->SortField == $Page->user_level->Name) { ?><?php if ($Page->SortOrder == "ASC") { ?><i class="fas fa-sort-up"></i><?php } elseif ($Page->SortOrder == "DESC") { ?><i class="fas fa-sort-down"></i><?php } ?><?php } ?></span>
        </div></div></th>
    <?php } ?>
<?php } ?>
