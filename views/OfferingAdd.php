...
loadjs.ready("head", function () {
    ...
    fofferingadd = currentForm = new ew.Form("fofferingadd", "add");

    // Add fields
    var fields = ew.vars.tables.offering.fields;
    fofferingadd.addFields([
    ...
    // Validate form
    fofferingadd.validate = function () {
    ...
    // Form_CustomValidate
    fofferingadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fofferingadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fofferingadd.lists.customer_id = <?= $Page->customer_id->toClientList($Page) ?>;
    loadjs.done("fofferingadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fofferingadd" id="fofferingadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<script>
loadjs.ready(["fofferingadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fofferingadd", "x_offering_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<?php
    if (in_array("offering_detail", explode(",", $Page->getCurrentDetailTable())) && $offering_detail->DetailAdd) {
?>
