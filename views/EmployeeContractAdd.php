...
var femployee_contractadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    femployee_contractadd = currentForm = new ew.Form("femployee_contractadd", "add");

    // Add fields
    var fields = ew.vars.tables.employee_contract.fields;
    femployee_contractadd.addFields([
    ...
    // Validate form
    femployee_contractadd.validate = function () {
    ...
    // Form_CustomValidate
    femployee_contractadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    femployee_contractadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    femployee_contractadd.lists.employee_username = <?= $Page->employee_username->toClientList($Page) ?>;
    femployee_contractadd.lists.office_id = <?= $Page->office_id->toClientList($Page) ?>;
    loadjs.done("femployee_contractadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="femployee_contractadd" id="femployee_contractadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->thr->isInvalidClass() ?>" data-table="employee_contract" data-field="x_thr" name="x_thr[]" id="x_thr_823835" value="1"<?= ConvertToBool($Page->thr->CurrentValue) ? " checked" : "" ?><?= $Page->thr->editAttributes() ?> aria-describedby="x_thr_help">
    <label class="custom-control-label" for="x_thr_823835"></label>
</div>
...
<script>
loadjs.ready(["femployee_contractadd", "datetimepicker"], function() {
    ew.createDateTimePicker("femployee_contractadd", "x_contract_start", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["femployee_contractadd", "datetimepicker"], function() {
    ew.createDateTimePicker("femployee_contractadd", "x_contract_end", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
