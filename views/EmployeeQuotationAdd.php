...
var femployee_quotationadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    femployee_quotationadd = currentForm = new ew.Form("femployee_quotationadd", "add");

    // Add fields
    var fields = ew.vars.tables.employee_quotation.fields;
    femployee_quotationadd.addFields([
    ...
    // Validate form
    femployee_quotationadd.validate = function () {
    ...
    // Form_CustomValidate
    femployee_quotationadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    femployee_quotationadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    femployee_quotationadd.lists.customer_id = <?= $Page->customer_id->toClientList($Page) ?>;
    loadjs.done("femployee_quotationadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="femployee_quotationadd" id="femployee_quotationadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<script>
loadjs.ready(["femployee_quotationadd", "datetimepicker"], function() {
    ew.createDateTimePicker("femployee_quotationadd", "x_quotation_date", {"ignoreReadonly":true,"useCurrent":false,"format":5});
});
</script>
<?php } ?>
</span>
...
<?php
    if (in_array("employee_quotation_detail", explode(",", $Page->getCurrentDetailTable())) && $employee_quotation_detail->DetailAdd) {
?>
