 ...
 * Page class
 */
class MasterProvinceAdd extends MasterProvince
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MasterProvinceAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("masterprovincelist"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "masterprovinceview") {
    ...
    protected function validateForm()
    {
        ...
        if (in_array("master_city", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->province->CurrentValue != "") { // Check field with unique index
            $filter = "(`province` = '" . AdjustSql($this->province->CurrentValue, $this->Dbid) . "')";
        ...
        if ($insertRow && $this->ValidateKey && strval($rsnew['province_id']) == "") {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("MasterCityGrid");
            if (in_array("master_city", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->province_id->setSessionValue($this->province_id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "master_city"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->province_id->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
