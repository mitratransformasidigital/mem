 ...
 * Page class
 */
class MasterCityList extends MasterCity
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MasterCityList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fmaster_citylist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "mastercityadd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "mastercitydelete";
        $this->MultiUpdateUrl = "mastercityupdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fmaster_citylistsrch";
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "master_province") {
            $masterTbl = Container("master_province");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("masterprovincelist"); // Return to master page
                return;
            } else {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmaster_citylistsrch", $filters);
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`city_id` ASC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->city_id->getSort() != "") {
                    $useDefaultSort = false;
                }
                if ($useDefaultSort) {
                    $this->city_id->setSort("ASC");
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_master_office"
        $item = &$this->ListOptions->add("detail_master_office");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'master_office') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        // "detail_master_office"
        $opt = $this->ListOptions["detail_master_office"];
        if ($Security->allowList(CurrentProjectID() . 'master_office')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("master_office", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("masterofficelist?" . Config("TABLE_SHOW_MASTER") . "=master_city&" . GetForeignKeyUrl("fk_city_id", $this->city_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("MasterOfficeGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'master_city')) {
                ...
                $detailViewTblVar .= "master_office";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'master_city')) {
                ...
                $detailEditTblVar .= "master_office";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'master_city')) {
                ...
                $detailCopyTblVar .= "master_office";
            }
            if ($links != "") {
        ...
        if ($Security->allowList(CurrentProjectID() . 'myprofile')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("myprofilelist?" . Config("TABLE_SHOW_MASTER") . "=master_city&" . GetForeignKeyUrl("fk_city_id", $this->city_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_master_office");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=master_office");
                $detailPage = Container("MasterOfficeGrid");
        ...
        $item->Body = "<a class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.fmaster_citylist, url:'" . GetUrl($this->MultiDeleteUrl) . "', data:{action:'show'}});return false;\">" . $Language->phrase("DeleteSelectedLink") . "</a>";
        $item->Visible = $Security->canDelete();

        // Add multi update
        $item = &$option->add("multiupdate");
        $item->Body = "<a class=\"ew-action ew-multi-update\" title=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" data-table=\"master_city\" data-caption=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.fmaster_citylist,url:'" . GetUrl($this->MultiUpdateUrl) . "'});return false;\">" . $Language->phrase("UpdateSelectedLink") . "</a>";
        $item->Visible = $Security->canEdit();
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmaster_citylistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmaster_citylistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fmaster_citylist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $curVal = trim(strval($this->province_id->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->province_id->AdvancedSearch->ViewValue = $this->province_id->lookupCacheOption($curVal);
            } else {
                $this->province_id->AdvancedSearch->ViewValue = $this->province_id->Lookup !== null && is_array($this->province_id->Lookup->Options) ? $curVal : null;
            }
            if ($this->province_id->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->province_id->EditValue = array_values($this->province_id->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`province_id`" . SearchString("=", $this->province_id->AdvancedSearch->SearchValue, DATATYPE_STRING, "");
                }
            ...
            if (!$this->city_id->Raw) {
                $this->city_id->AdvancedSearch->SearchValue = HtmlDecode($this->city_id->AdvancedSearch->SearchValue);
            }
            $this->city_id->EditValue = HtmlEncode($this->city_id->AdvancedSearch->SearchValue);
            $this->city_id->PlaceHolder = RemoveHtml($this->city_id->caption());
            $this->city_id->EditAttrs["class"] = "form-control";
            $this->city_id->EditCustomAttributes = "";
            if (!$this->city_id->Raw) {
                $this->city_id->AdvancedSearch->SearchValue2 = HtmlDecode($this->city_id->AdvancedSearch->SearchValue2);
            }
            $this->city_id->EditValue2 = HtmlEncode($this->city_id->AdvancedSearch->SearchValue2);
            $this->city_id->PlaceHolder = RemoveHtml($this->city_id->caption());
            ...
            $this->city->EditCustomAttributes = "";
            if (!$this->city->Raw) {
                $this->city->AdvancedSearch->SearchValue2 = HtmlDecode($this->city->AdvancedSearch->SearchValue2);
            }
            $this->city->EditValue2 = HtmlEncode($this->city->AdvancedSearch->SearchValue2);
            $this->city->PlaceHolder = RemoveHtml($this->city->caption());
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fmaster_citylist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fmaster_citylist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fmaster_citylist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_master_city" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_master_city\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fmaster_citylist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fmaster_citylistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"mastercitysearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"master_city\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SearchBtn',url:'mastercitysearch'});\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "master_province") {
            $master_province = Container("master_province");
            $rsmaster = $master_province->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $master_province;
                    $master_province->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "master_province") {
                $validMaster = true;
                $masterTbl = Container("master_province");
                if (($parm = Get("fk_province_id", Get("province_id"))) !== null) {
                    $masterTbl->province_id->setQueryStringValue($parm);
                    $this->province_id->setQueryStringValue($masterTbl->province_id->QueryStringValue);
            ...
            if ($masterTblVar == "master_province") {
                $validMaster = true;
                $masterTbl = Container("master_province");
                if (($parm = Post("fk_province_id", Post("province_id"))) !== null) {
                    $masterTbl->province_id->setFormValue($parm);
                    $this->province_id->setFormValue($masterTbl->province_id->FormValue);
