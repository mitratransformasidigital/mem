 ...
 * Page class
 */
class OfferingEdit extends Offering
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "OfferingEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("offering_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->offering_id->setQueryStringValue($keyValue);
                $this->offering_id->setOldValue($this->offering_id->QueryStringValue);
            } elseif (Post("offering_id") !== null) {
                $this->offering_id->setFormValue(Post("offering_id"));
                $this->offering_id->setOldValue($this->offering_id->FormValue);
            } else {
            ...
            if ($this->isShow()) {
                ...
                if ($this->TotalRecords <= 0) { // No record found
                    ...
                    if ($this->offering_id->CurrentValue != null) {
                        while (!$rs->EOF) {
                            if (SameString($this->offering_id->CurrentValue, $rs->fields['offering_id'])) {
    ...
    protected function validateForm()
    {
        ...
        if (in_array("offering_detail", $detailTblVar) && $detailPage->DetailEdit) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // offering_no
            $this->offering_no->setDbValueDef($rsnew, $this->offering_no->CurrentValue, "", $this->offering_no->ReadOnly);

            // customer_id
            $this->customer_id->setDbValueDef($rsnew, $this->customer_id->CurrentValue, 0, $this->customer_id->ReadOnly);

            // offering_date
            $this->offering_date->setDbValueDef($rsnew, UnFormatDateTime($this->offering_date->CurrentValue, 0), CurrentDate(), $this->offering_date->ReadOnly);

            // offering_term
            $this->offering_term->setDbValueDef($rsnew, $this->offering_term->CurrentValue, "", $this->offering_term->ReadOnly);
