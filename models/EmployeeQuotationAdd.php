 ...
 * Page class
 */
class EmployeeQuotationAdd extends EmployeeQuotation
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeeQuotationAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("employeequotationlist"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "employeequotationview") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->quotation_no->OldValue = $this->quotation_no->CurrentValue;
        $this->customer_id->CurrentValue = null;
        $this->customer_id->OldValue = $this->customer_id->CurrentValue;
        $this->quotation_date->CurrentValue = CurrentDate();
    }
    ...
    protected function newRow()
    {
        ...
        $row['quotation_no'] = $this->quotation_no->CurrentValue;
        $row['customer_id'] = $this->customer_id->CurrentValue;
        $row['quotation_date'] = $this->quotation_date->CurrentValue;
        return $row;
    }
    ...
    protected function validateForm()
    {
        ...
        if (in_array("employee_quotation_detail", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->quotation_no->CurrentValue != "") { // Check field with unique index
            $filter = "(`quotation_no` = '" . AdjustSql($this->quotation_no->CurrentValue, $this->Dbid) . "')";
        ...
        // quotation_no
        $this->quotation_no->setDbValueDef($rsnew, $this->quotation_no->CurrentValue, "", false);

        // customer_id
        $this->customer_id->setDbValueDef($rsnew, $this->customer_id->CurrentValue, 0, false);

        // quotation_date
        $this->quotation_date->setDbValueDef($rsnew, UnFormatDateTime($this->quotation_date->CurrentValue, 5), CurrentDate(), false);
        ...
        if ($addRow) {
            ...
            $detailPage = Container("EmployeeQuotationDetailGrid");
            if (in_array("employee_quotation_detail", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->quotation_id->setSessionValue($this->quotation_id->CurrentValue); // Set master key
                ...
                if (!$addRow) {
                $detailPage->quotation_id->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
