...
/**
 * Table class for offering
 */
class Offering extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'offering';
        $this->TableName = 'offering';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`offering`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // offering_id
        $this->offering_id = new DbField('offering', 'offering', 'x_offering_id', 'offering_id', '`offering_id`', '`offering_id`', 3, 11, -1, false, '`offering_id`', false, false, false, 'FORMATTED TEXT', 'HIDDEN');
        ...
        // offering_no
        $this->offering_no = new DbField('offering', 'offering', 'x_offering_no', 'offering_no', '`offering_no`', '`offering_no`', 200, 100, -1, false, '`offering_no`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // customer_id
        $this->customer_id = new DbField('offering', 'offering', 'x_customer_id', 'customer_id', '`customer_id`', '`customer_id`', 3, 11, -1, false, '`customer_id`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        // offering_date
        $this->offering_date = new DbField('offering', 'offering', 'x_offering_date', 'offering_date', '`offering_date`', CastDateFieldForLike("`offering_date`", 0, "DB"), 133, 10, 0, false, '`offering_date`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->offering_date->Nullable = false; // NOT NULL field
        $this->offering_date->Required = true; // Required field
        $this->offering_date->Sortable = true; // Allow sort
        $this->offering_date->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
        $this->Fields['offering_date'] = &$this->offering_date;

        // offering_term
        $this->offering_term = new DbField('offering', 'offering', 'x_offering_term', 'offering_term', '`offering_term`', '`offering_term`', 201, 5000, -1, false, '`offering_term`', false, false, false, 'FORMATTED TEXT', 'TEXTAREA');
        $this->offering_term->Nullable = false; // NOT NULL field
        $this->offering_term->Required = true; // Required field
        $this->offering_term->Sortable = true; // Allow sort
        $this->Fields['offering_term'] = &$this->offering_term;
    }
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "offering_detail") {
            $detailUrl = Container("offering_detail")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_offering_id", $this->offering_id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "offeringlist";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`offering`";
    }
    ...
    {
        // Cascade Update detail table 'offering_detail'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['offering_id']) && $rsold['offering_id'] != $rs['offering_id'])) { // Update detail field 'offering_id'
            $cascadeUpdate = true;
            $rscascade['offering_id'] = $rs['offering_id'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("offering_detail")->loadRs("`offering_id` = " . QuotedValue($rsold['offering_id'], DATATYPE_NUMBER, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("offering_detail")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("offering_detail")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("offering_detail")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'offering_detail'
        $dtlrows = Container("offering_detail")->loadRs("`offering_id` = " . QuotedValue($rs['offering_id'], DATATYPE_NUMBER, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("offering_detail")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("offering_detail")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("offering_detail")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->offering_term->DbValue = $row['offering_term'];
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("offeringlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "offeringedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "offeringadd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("offeringview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("offeringview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "offeringadd?" . $this->getUrlParm($parm);
        } else {
            $url = "offeringadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("offeringedit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("offeringedit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("offeringadd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("offeringadd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("offeringdelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $this->offering_date->EditValue = FormatDateTime($this->offering_date->CurrentValue, 8);
        ...
        $this->offering_term->EditValue = $this->offering_term->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->offering_term);
                } else {
                    $doc->exportCaption($this->offering_no);
                    $doc->exportCaption($this->customer_id);
                    $doc->exportCaption($this->offering_date);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->offering_term);
                    } else {
                        $doc->exportField($this->offering_no);
                        $doc->exportField($this->customer_id);
                        $doc->exportField($this->offering_date);
    ...
    public function rowRendering()
    {
        // Enter your code here
            ...
            if (CurrentPageID() == "add") {
                 $conn = $c ?? $this->getConnection();
                 $rs = $conn->executeQuery("SELECT max(offering_id) from `offering`");
                 $cnt = $rs->fetchColumn();
                 if ($cnt !== false) {
                      $val = (int)$cnt + 1;
                      $this->offering_no->CurrentValue = "MITRAL/".date("y")."/".date("m")."/O/".str_pad($val, 6, '0', STR_PAD_LEFT);  
                 } 
                 else
                 {
    		         $val = 1;
