 ...
 * Page class
 */
class EmployeeQuotationEdit extends EmployeeQuotation
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeeQuotationEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("quotation_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->quotation_id->setQueryStringValue($keyValue);
                $this->quotation_id->setOldValue($this->quotation_id->QueryStringValue);
            } elseif (Post("quotation_id") !== null) {
                $this->quotation_id->setFormValue(Post("quotation_id"));
                $this->quotation_id->setOldValue($this->quotation_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                if (($keyValue = Get("quotation_id") ?? Route("quotation_id")) !== null) {
                    ...
                    $this->quotation_id->CurrentValue = null;
                }
                if (!$loadByQuery) {
                    $loadByPosition = true;
                }
            ...
            if ($this->isShow()) {
                ...
                if ($this->TotalRecords <= 0) { // No record found
                    ...
                    if ($this->quotation_id->CurrentValue != null) {
                        while (!$rs->EOF) {
                            if (SameString($this->quotation_id->CurrentValue, $rs->fields['quotation_id'])) {
        ...
        switch ($this->CurrentAction) {
            ...
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "employeequotationlist") {
    ...
    protected function validateForm()
    {
        ...
        if ($this->quotation_no->Required) {
            if (!$this->quotation_no->IsDetailKey && EmptyValue($this->quotation_no->FormValue)) {
        ...
        if ($this->quotation_date->Required) {
            if (!$this->quotation_date->IsDetailKey && EmptyValue($this->quotation_date->FormValue)) {
        ...
        if (in_array("employee_quotation_detail", $detailTblVar) && $detailPage->DetailEdit) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // quotation_no
            $this->quotation_no->setDbValueDef($rsnew, $this->quotation_no->CurrentValue, "", $this->quotation_no->ReadOnly);

            // customer_id
            $this->customer_id->setDbValueDef($rsnew, $this->customer_id->CurrentValue, 0, $this->customer_id->ReadOnly);

            // quotation_date
            $this->quotation_date->setDbValueDef($rsnew, UnFormatDateTime($this->quotation_date->CurrentValue, 5), CurrentDate(), $this->quotation_date->ReadOnly);
            ...
            if ($updateRow) {
                ...
                if ($editRow) {
                    $detailPage = Container("EmployeeQuotationDetailGrid");
                    if (in_array("employee_quotation_detail", $detailTblVar) && $detailPage->DetailEdit) {
                        $Security->loadCurrentUserLevel($this->ProjectID . "employee_quotation_detail"); // Load user level of detail table
