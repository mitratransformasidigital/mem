...
/**
 * Table class for employee_quotation
 */
class EmployeeQuotation extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'employee_quotation';
        $this->TableName = 'employee_quotation';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`employee_quotation`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // quotation_id
        $this->quotation_id = new DbField('employee_quotation', 'employee_quotation', 'x_quotation_id', 'quotation_id', '`quotation_id`', '`quotation_id`', 3, 11, -1, false, '`quotation_id`', false, false, false, 'FORMATTED TEXT', 'HIDDEN');
        ...
        // quotation_no
        $this->quotation_no = new DbField('employee_quotation', 'employee_quotation', 'x_quotation_no', 'quotation_no', '`quotation_no`', '`quotation_no`', 200, 50, -1, false, '`quotation_no`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // customer_id
        $this->customer_id = new DbField('employee_quotation', 'employee_quotation', 'x_customer_id', 'customer_id', '`customer_id`', '`customer_id`', 3, 11, -1, false, '`customer_id`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        $this->customer_id->Lookup = new Lookup('customer_id', 'customer', false, 'customer_id', ["customer_name","","",""], [], [], [], [], [], [], '', '');
        $this->customer_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['customer_id'] = &$this->customer_id;

        // quotation_date
        $this->quotation_date = new DbField('employee_quotation', 'employee_quotation', 'x_quotation_date', 'quotation_date', '`quotation_date`', CastDateFieldForLike("`quotation_date`", 5, "DB"), 133, 10, 5, false, '`quotation_date`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "employee_quotation_detail") {
            $detailUrl = Container("employee_quotation_detail")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_quotation_id", $this->quotation_id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "employeequotationlist";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`employee_quotation`";
    }
    ...
    {
        // Cascade Update detail table 'employee_quotation_detail'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['quotation_id']) && $rsold['quotation_id'] != $rs['quotation_id'])) { // Update detail field 'quotation_id'
            $cascadeUpdate = true;
            $rscascade['quotation_id'] = $rs['quotation_id'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("employee_quotation_detail")->loadRs("`quotation_id` = " . QuotedValue($rsold['quotation_id'], DATATYPE_NUMBER, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("employee_quotation_detail")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("employee_quotation_detail")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("employee_quotation_detail")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'employee_quotation_detail'
        $dtlrows = Container("employee_quotation_detail")->loadRs("`quotation_id` = " . QuotedValue($rs['quotation_id'], DATATYPE_NUMBER, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("employee_quotation_detail")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("employee_quotation_detail")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("employee_quotation_detail")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("employeequotationlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "employeequotationedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "employeequotationadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "EmployeeQuotationView";
            case Config("API_ADD_ACTION"):
                return "EmployeeQuotationAdd";
            case Config("API_EDIT_ACTION"):
                return "EmployeeQuotationEdit";
            case Config("API_DELETE_ACTION"):
                return "EmployeeQuotationDelete";
            case Config("API_LIST_ACTION"):
                return "EmployeeQuotationList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("employeequotationview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("employeequotationview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "employeequotationadd?" . $this->getUrlParm($parm);
        } else {
            $url = "employeequotationadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("employeequotationedit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("employeequotationedit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("employeequotationadd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("employeequotationadd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("employeequotationdelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $this->quotation_date->EditValue = FormatDateTime($this->quotation_date->CurrentValue, 5);
    ...
    public function rowRendering()
    {
        // Enter your code here
            ...
            if (CurrentPageID() == "add") {
                 $conn = $c ?? $this->getConnection();
                 $rs = $conn->executeQuery("SELECT max(quotation_id) from `employee_quotation`");
                 $cnt = $rs->fetchColumn();
                 if ($cnt !== false) {
                      $val = (int)$cnt + 1;
                      $this->quotation_no->CurrentValue = "MITRAL/".date("y")."/".date("m")."/Q/".str_pad($val, 6, '0', STR_PAD_LEFT);  
                 } 
                 else
                 {
    		         $val = 1;
