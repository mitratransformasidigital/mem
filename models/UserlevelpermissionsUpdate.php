 ...
 * Page class
 */
class UserlevelpermissionsUpdate extends Userlevelpermissions
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "UserlevelpermissionsUpdate";
    ...
    public function run()
    {
        ...
        $this->_tablename->Required = false;
        ...
        if (count($this->RecKeys) <= 0) {
            $this->terminate("userlevelpermissionslist"); // No records selected, return to list
            return;
        }
        if ($this->isUpdate()) {
        ...
        if ($rs = $this->loadRecordset()) {
            ...
            while (!$rs->EOF) {
                if ($i == 1) {
                    ...
                    $this->_permission->setDbValue($rs->fields['permission']);
                } else {
                    if (!CompareValue($this->userlevelid->DbValue, $rs->fields['userlevelid'])) {
                        $this->userlevelid->CurrentValue = null;
                    }
                    if (!CompareValue($this->_tablename->DbValue, $rs->fields['tablename'])) {
                        $this->_tablename->CurrentValue = null;
                    }
                    if (!CompareValue($this->_permission->DbValue, $rs->fields['permission'])) {
    ...
    protected function setupKeyValues($key)
    {
        ...
        $this->userlevelid->OldValue = $keyFld;
        $keyFld = $key[1];
        $this->_tablename->OldValue = $keyFld;
        return true;
    }
    ...
    protected function loadFormValues()
    {
        ...
        $this->userlevelid->MultiUpdate = $CurrentForm->getValue("u_userlevelid");
        ...
        $this->_tablename->MultiUpdate = $CurrentForm->getValue("u__tablename");
        ...
        $this->_permission->MultiUpdate = $CurrentForm->getValue("u__permission");
    }
    ...
    protected function validateForm()
    {
        global $Language;
        $updateCnt = 0;
        if ($this->userlevelid->multiUpdateSelected()) {
            $updateCnt++;
        }
        if ($this->_tablename->multiUpdateSelected()) {
            $updateCnt++;
        }
        if ($this->_permission->multiUpdateSelected()) {
            $updateCnt++;
        }
        if ($updateCnt == 0) {
            return false;
        ...
        if ($this->userlevelid->Required) {
            if ($this->userlevelid->MultiUpdate != "" && !$this->userlevelid->IsDetailKey && EmptyValue($this->userlevelid->FormValue)) {
        ...
        if ($this->_tablename->Required) {
            if ($this->_tablename->MultiUpdate != "" && !$this->_tablename->IsDetailKey && EmptyValue($this->_tablename->FormValue)) {
        ...
        if ($this->_permission->Required) {
            if ($this->_permission->MultiUpdate != "" && !$this->_permission->IsDetailKey && EmptyValue($this->_permission->FormValue)) {
                $this->_permission->addErrorMessage(str_replace("%s", $this->_permission->caption(), $this->_permission->RequiredErrorMessage));
            }
        }
        if ($this->_permission->MultiUpdate != "") {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // userlevelid
            $this->userlevelid->setDbValueDef($rsnew, $this->userlevelid->CurrentValue, 0, $this->userlevelid->ReadOnly || $this->userlevelid->MultiUpdate != "1");

            // tablename
            $this->_tablename->setDbValueDef($rsnew, $this->_tablename->CurrentValue, "", $this->_tablename->ReadOnly || $this->_tablename->MultiUpdate != "1");

            // permission
            $this->_permission->setDbValueDef($rsnew, $this->_permission->CurrentValue, 0, $this->_permission->ReadOnly || $this->_permission->MultiUpdate != "1");
