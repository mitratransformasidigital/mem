 ...
 * Page class
 */
class PermitEdit extends Permit
{
    use MessagesTrait;
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("permit_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->permit_id->setQueryStringValue($keyValue);
                $this->permit_id->setOldValue($this->permit_id->QueryStringValue);
            } elseif (Post("permit_id") !== null) {
                $this->permit_id->setFormValue(Post("permit_id"));
                $this->permit_id->setOldValue($this->permit_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                if (($keyValue = Get("permit_id") ?? Route("permit_id")) !== null) {
                    ...
                    $this->permit_id->CurrentValue = null;
                }
                if (!$loadByQuery) {
                    $loadByPosition = true;
                }
            ...
            if ($this->isShow()) {
                ...
                if ($this->TotalRecords <= 0) { // No record found
                    ...
                    if ($this->permit_id->CurrentValue != null) {
                        while (!$rs->EOF) {
                            if (SameString($this->permit_id->CurrentValue, $rs->fields['permit_id'])) {
        ...
        switch ($this->CurrentAction) {
            ...
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "permitlist") {
    ...
    protected function validateForm()
    {
        ...
        if ($this->permit_id->Required) {
            if (!$this->permit_id->IsDetailKey && EmptyValue($this->permit_id->FormValue)) {
                $this->permit_id->addErrorMessage(str_replace("%s", $this->permit_id->caption(), $this->permit_id->RequiredErrorMessage));
            }
        }
