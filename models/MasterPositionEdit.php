 ...
 * Page class
 */
class MasterPositionEdit extends MasterPosition
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MasterPositionEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("position_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->position_id->setQueryStringValue($keyValue);
                $this->position_id->setOldValue($this->position_id->QueryStringValue);
            } elseif (Post("position_id") !== null) {
                $this->position_id->setFormValue(Post("position_id"));
                $this->position_id->setOldValue($this->position_id->FormValue);
            } else {
            ...
            if ($this->isShow()) {
                ...
                if ($this->TotalRecords <= 0) { // No record found
                    ...
                    if ($this->position_id->CurrentValue != null) {
                        while (!$rs->EOF) {
                            if (SameString($this->position_id->CurrentValue, $rs->fields['position_id'])) {
