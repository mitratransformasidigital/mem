 ...
 * Page class
 */
class EmployeeAssetGrid extends EmployeeAsset
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeeAssetGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "femployee_assetgrid";
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_asset_name") && $CurrentForm->hasValue("o_asset_name") && $this->asset_name->CurrentValue != $this->asset_name->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_year") && $CurrentForm->hasValue("o_year") && $this->year->CurrentValue != $this->year->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_serial_number") && $CurrentForm->hasValue("o_serial_number") && $this->serial_number->CurrentValue != $this->serial_number->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_value") && $CurrentForm->hasValue("o_value") && $this->value->CurrentValue != $this->value->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_asset_received") && $CurrentForm->hasValue("o_asset_received") && $this->asset_received->CurrentValue != $this->asset_received->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_asset_return") && $CurrentForm->hasValue("o_asset_return") && $this->asset_return->CurrentValue != $this->asset_return->OldValue) {
            return false;
        }
        if (!EmptyValue($this->asset_picture->Upload->Value)) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->asset_name->clearErrorMessage();
        $this->year->clearErrorMessage();
        $this->serial_number->clearErrorMessage();
        $this->value->clearErrorMessage();
        $this->asset_received->clearErrorMessage();
        $this->asset_return->clearErrorMessage();
        $this->asset_picture->clearErrorMessage();
    }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->asset_picture->Upload->Index = $this->RowIndex;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_asset_name")) {
            $this->asset_name->setOldValue($CurrentForm->getValue("o_asset_name"));
        }
        ...
        if ($CurrentForm->hasValue("o_serial_number")) {
            $this->serial_number->setOldValue($CurrentForm->getValue("o_serial_number"));
        }
        ...
        if ($CurrentForm->hasValue("o_asset_received")) {
            $this->asset_received->setOldValue($CurrentForm->getValue("o_asset_received"));
        }
        ...
        if ($CurrentForm->hasValue("o_asset_return")) {
            $this->asset_return->setOldValue($CurrentForm->getValue("o_asset_return"));
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (is_numeric($this->RowIndex)) {
                RenderUploadField($this->asset_picture, $this->RowIndex);
            }
