 ...
 * Page class
 */
class EmployeeTrainingsList extends EmployeeTrainings
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'employee_trainings';

    // Page object name
    public $PageObjName = "EmployeeTrainingsList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "femployee_trainingslist";
    ...
    public function __construct()
    {
        ...
        // Table object (employee_trainings)
        if (!isset($GLOBALS["employee_trainings"]) || get_class($GLOBALS["employee_trainings"]) == PROJECT_NAMESPACE . "employee_trainings") {
            $GLOBALS["employee_trainings"] = &$this;
        }
        ...
        $this->AddUrl = "employeetrainingsadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "employeetrainingsdelete";
        $this->MultiUpdateUrl = "employeetrainingsupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'employee_trainings');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option femployee_trainingslistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("employee_trainings"));
                $doc->Text = @$content;
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->training_id->AdvancedSearch->toJson(), ","); // Field training_id
        $filterList = Concat($filterList, $this->employee_username->AdvancedSearch->toJson(), ","); // Field employee_username
        $filterList = Concat($filterList, $this->training_name->AdvancedSearch->toJson(), ","); // Field training_name
        $filterList = Concat($filterList, $this->training_start->AdvancedSearch->toJson(), ","); // Field training_start
        $filterList = Concat($filterList, $this->training_end->AdvancedSearch->toJson(), ","); // Field training_end
        $filterList = Concat($filterList, $this->training_company->AdvancedSearch->toJson(), ","); // Field training_company
        $filterList = Concat($filterList, $this->certificate_start->AdvancedSearch->toJson(), ","); // Field certificate_start
        $filterList = Concat($filterList, $this->certificate_end->AdvancedSearch->toJson(), ","); // Field certificate_end
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "femployee_trainingslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field training_id
        $this->training_id->AdvancedSearch->SearchValue = @$filter["x_training_id"];
        $this->training_id->AdvancedSearch->SearchOperator = @$filter["z_training_id"];
        $this->training_id->AdvancedSearch->SearchCondition = @$filter["v_training_id"];
        $this->training_id->AdvancedSearch->SearchValue2 = @$filter["y_training_id"];
        $this->training_id->AdvancedSearch->SearchOperator2 = @$filter["w_training_id"];
        $this->training_id->AdvancedSearch->save();
        ...
        // Field training_name
        $this->training_name->AdvancedSearch->SearchValue = @$filter["x_training_name"];
        $this->training_name->AdvancedSearch->SearchOperator = @$filter["z_training_name"];
        $this->training_name->AdvancedSearch->SearchCondition = @$filter["v_training_name"];
        $this->training_name->AdvancedSearch->SearchValue2 = @$filter["y_training_name"];
        $this->training_name->AdvancedSearch->SearchOperator2 = @$filter["w_training_name"];
        $this->training_name->AdvancedSearch->save();

        // Field training_start
        $this->training_start->AdvancedSearch->SearchValue = @$filter["x_training_start"];
        $this->training_start->AdvancedSearch->SearchOperator = @$filter["z_training_start"];
        $this->training_start->AdvancedSearch->SearchCondition = @$filter["v_training_start"];
        $this->training_start->AdvancedSearch->SearchValue2 = @$filter["y_training_start"];
        $this->training_start->AdvancedSearch->SearchOperator2 = @$filter["w_training_start"];
        $this->training_start->AdvancedSearch->save();

        // Field training_end
        $this->training_end->AdvancedSearch->SearchValue = @$filter["x_training_end"];
        $this->training_end->AdvancedSearch->SearchOperator = @$filter["z_training_end"];
        $this->training_end->AdvancedSearch->SearchCondition = @$filter["v_training_end"];
        $this->training_end->AdvancedSearch->SearchValue2 = @$filter["y_training_end"];
        $this->training_end->AdvancedSearch->SearchOperator2 = @$filter["w_training_end"];
        $this->training_end->AdvancedSearch->save();

        // Field training_company
        $this->training_company->AdvancedSearch->SearchValue = @$filter["x_training_company"];
        $this->training_company->AdvancedSearch->SearchOperator = @$filter["z_training_company"];
        $this->training_company->AdvancedSearch->SearchCondition = @$filter["v_training_company"];
        $this->training_company->AdvancedSearch->SearchValue2 = @$filter["y_training_company"];
        $this->training_company->AdvancedSearch->SearchOperator2 = @$filter["w_training_company"];
        $this->training_company->AdvancedSearch->save();

        // Field certificate_start
        $this->certificate_start->AdvancedSearch->SearchValue = @$filter["x_certificate_start"];
        $this->certificate_start->AdvancedSearch->SearchOperator = @$filter["z_certificate_start"];
        $this->certificate_start->AdvancedSearch->SearchCondition = @$filter["v_certificate_start"];
        $this->certificate_start->AdvancedSearch->SearchValue2 = @$filter["y_certificate_start"];
        $this->certificate_start->AdvancedSearch->SearchOperator2 = @$filter["w_certificate_start"];
        $this->certificate_start->AdvancedSearch->save();

        // Field certificate_end
        $this->certificate_end->AdvancedSearch->SearchValue = @$filter["x_certificate_end"];
        $this->certificate_end->AdvancedSearch->SearchOperator = @$filter["z_certificate_end"];
        $this->certificate_end->AdvancedSearch->SearchCondition = @$filter["v_certificate_end"];
        $this->certificate_end->AdvancedSearch->SearchValue2 = @$filter["y_certificate_end"];
        $this->certificate_end->AdvancedSearch->SearchOperator2 = @$filter["w_certificate_end"];
        $this->certificate_end->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->training_id, $default, false); // training_id
        $this->buildSearchSql($where, $this->employee_username, $default, false); // employee_username
        $this->buildSearchSql($where, $this->training_name, $default, false); // training_name
        $this->buildSearchSql($where, $this->training_start, $default, false); // training_start
        $this->buildSearchSql($where, $this->training_end, $default, false); // training_end
        $this->buildSearchSql($where, $this->training_company, $default, false); // training_company
        $this->buildSearchSql($where, $this->certificate_start, $default, false); // certificate_start
        $this->buildSearchSql($where, $this->certificate_end, $default, false); // certificate_end
        ...
        if (!$default && $this->Command == "search") {
            $this->training_id->AdvancedSearch->save(); // training_id
            $this->employee_username->AdvancedSearch->save(); // employee_username
            $this->training_name->AdvancedSearch->save(); // training_name
            $this->training_start->AdvancedSearch->save(); // training_start
            $this->training_end->AdvancedSearch->save(); // training_end
            $this->training_company->AdvancedSearch->save(); // training_company
            $this->certificate_start->AdvancedSearch->save(); // certificate_start
            $this->certificate_end->AdvancedSearch->save(); // certificate_end
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->training_name, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->training_company, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->notes, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->training_document, $arKeywords, $type);
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->training_id->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->employee_username->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->training_name->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->training_start->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->training_end->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->training_company->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->certificate_start->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->certificate_end->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
                $this->training_id->AdvancedSearch->unsetSession();
                $this->employee_username->AdvancedSearch->unsetSession();
                $this->training_name->AdvancedSearch->unsetSession();
                $this->training_start->AdvancedSearch->unsetSession();
                $this->training_end->AdvancedSearch->unsetSession();
                $this->training_company->AdvancedSearch->unsetSession();
                $this->certificate_start->AdvancedSearch->unsetSession();
                $this->certificate_end->AdvancedSearch->unsetSession();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->training_name); // training_name
            $this->updateSort($this->training_start); // training_start
            $this->updateSort($this->training_end); // training_end
            $this->updateSort($this->training_company); // training_company
            $this->updateSort($this->certificate_start); // certificate_start
            $this->updateSort($this->certificate_end); // certificate_end
            $this->updateSort($this->training_document); // training_document
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->training_id->setSort("");
                $this->employee_username->setSort("");
                $this->training_name->setSort("");
                $this->training_start->setSort("");
                $this->training_end->setSort("");
                $this->training_company->setSort("");
                $this->certificate_start->setSort("");
                $this->certificate_end->setSort("");
                $this->notes->setSort("");
                $this->training_document->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->training_id->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.femployee_trainingslist, url:'" . GetUrl($this->MultiDeleteUrl) . "', data:{action:'show'}});return false;\">" . $Language->phrase("DeleteSelectedLink") . "</a>";
        $item->Visible = $Security->canDelete();

        // Add multi update
        $item = &$option->add("multiupdate");
        $item->Body = "<a class=\"ew-action ew-multi-update\" title=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" data-table=\"employee_trainings\" data-caption=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.femployee_trainingslist,url:'" . GetUrl($this->MultiUpdateUrl) . "'});return false;\">" . $Language->phrase("UpdateSelectedLink") . "</a>";
        $item->Visible = $Security->canEdit();
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"femployee_trainingslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"femployee_trainingslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.femployee_trainingslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // training_id
        if (!$this->isAddOrEdit() && $this->training_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->training_id->AdvancedSearch->SearchValue != "" || $this->training_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // training_name
        if (!$this->isAddOrEdit() && $this->training_name->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->training_name->AdvancedSearch->SearchValue != "" || $this->training_name->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // training_start
        if (!$this->isAddOrEdit() && $this->training_start->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->training_start->AdvancedSearch->SearchValue != "" || $this->training_start->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // training_end
        if (!$this->isAddOrEdit() && $this->training_end->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->training_end->AdvancedSearch->SearchValue != "" || $this->training_end->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // training_company
        if (!$this->isAddOrEdit() && $this->training_company->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->training_company->AdvancedSearch->SearchValue != "" || $this->training_company->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // certificate_start
        if (!$this->isAddOrEdit() && $this->certificate_start->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->certificate_start->AdvancedSearch->SearchValue != "" || $this->certificate_start->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // certificate_end
        if (!$this->isAddOrEdit() && $this->certificate_end->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->certificate_end->AdvancedSearch->SearchValue != "" || $this->certificate_end->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.femployee_trainingslist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.femployee_trainingslist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.femployee_trainingslist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_employee_trainings" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_employee_trainings\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.femployee_trainingslist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"femployee_trainingslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"employeetrainingssearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"employee_trainings\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SearchBtn',url:'employeetrainingssearch'});\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
