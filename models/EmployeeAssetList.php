 ...
 * Page class
 */
class EmployeeAssetList extends EmployeeAsset
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeeAssetList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "femployee_assetlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "employeeassetadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "employeeassetdelete";
        $this->MultiUpdateUrl = "employeeassetupdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option femployee_assetlistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->asset_name->AdvancedSearch->toJson(), ","); // Field asset_name
        ...
        $filterList = Concat($filterList, $this->asset_received->AdvancedSearch->toJson(), ","); // Field asset_received
        $filterList = Concat($filterList, $this->asset_return->AdvancedSearch->toJson(), ","); // Field asset_return
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "femployee_assetlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field asset_name
        $this->asset_name->AdvancedSearch->SearchValue = @$filter["x_asset_name"];
        $this->asset_name->AdvancedSearch->SearchOperator = @$filter["z_asset_name"];
        $this->asset_name->AdvancedSearch->SearchCondition = @$filter["v_asset_name"];
        $this->asset_name->AdvancedSearch->SearchValue2 = @$filter["y_asset_name"];
        $this->asset_name->AdvancedSearch->SearchOperator2 = @$filter["w_asset_name"];
        $this->asset_name->AdvancedSearch->save();
        ...
        // Field asset_received
        $this->asset_received->AdvancedSearch->SearchValue = @$filter["x_asset_received"];
        $this->asset_received->AdvancedSearch->SearchOperator = @$filter["z_asset_received"];
        $this->asset_received->AdvancedSearch->SearchCondition = @$filter["v_asset_received"];
        $this->asset_received->AdvancedSearch->SearchValue2 = @$filter["y_asset_received"];
        $this->asset_received->AdvancedSearch->SearchOperator2 = @$filter["w_asset_received"];
        $this->asset_received->AdvancedSearch->save();

        // Field asset_return
        $this->asset_return->AdvancedSearch->SearchValue = @$filter["x_asset_return"];
        $this->asset_return->AdvancedSearch->SearchOperator = @$filter["z_asset_return"];
        $this->asset_return->AdvancedSearch->SearchCondition = @$filter["v_asset_return"];
        $this->asset_return->AdvancedSearch->SearchValue2 = @$filter["y_asset_return"];
        $this->asset_return->AdvancedSearch->SearchOperator2 = @$filter["w_asset_return"];
        $this->asset_return->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->asset_name, $default, false); // asset_name
        $this->buildSearchSql($where, $this->year, $default, false); // year
        $this->buildSearchSql($where, $this->serial_number, $default, false); // serial_number
        $this->buildSearchSql($where, $this->value, $default, false); // value
        $this->buildSearchSql($where, $this->asset_received, $default, false); // asset_received
        $this->buildSearchSql($where, $this->asset_return, $default, false); // asset_return
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->asset_name->AdvancedSearch->save(); // asset_name
            $this->year->AdvancedSearch->save(); // year
            $this->serial_number->AdvancedSearch->save(); // serial_number
            $this->value->AdvancedSearch->save(); // value
            $this->asset_received->AdvancedSearch->save(); // asset_received
            $this->asset_return->AdvancedSearch->save(); // asset_return
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->asset_name, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->serial_number, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->asset_picture, $arKeywords, $type);
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->asset_name->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->year->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->serial_number->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->value->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->asset_received->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->asset_return->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
                ...
                $this->asset_name->AdvancedSearch->unsetSession();
                $this->year->AdvancedSearch->unsetSession();
                $this->serial_number->AdvancedSearch->unsetSession();
                $this->value->AdvancedSearch->unsetSession();
                $this->asset_received->AdvancedSearch->unsetSession();
                $this->asset_return->AdvancedSearch->unsetSession();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->asset_name); // asset_name
            $this->updateSort($this->year); // year
            $this->updateSort($this->serial_number); // serial_number
            $this->updateSort($this->value); // value
            $this->updateSort($this->asset_received); // asset_received
            $this->updateSort($this->asset_return); // asset_return
            $this->updateSort($this->asset_picture); // asset_picture
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`asset_received` DESC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->asset_received->getSort() != "") {
                    $useDefaultSort = false;
                }
                if ($useDefaultSort) {
                    $this->asset_received->setSort("DESC");
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->asset_name->setSort("");
                $this->year->setSort("");
                $this->serial_number->setSort("");
                $this->value->setSort("");
                $this->asset_received->setSort("");
                $this->asset_return->setSort("");
                $this->asset_picture->setSort("");
                $this->notes->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->asset_id->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.femployee_assetlist, url:'" . GetUrl($this->MultiDeleteUrl) . "', data:{action:'show'}});return false;\">" . $Language->phrase("DeleteSelectedLink") . "</a>";
        $item->Visible = $Security->canDelete();

        // Add multi update
        $item = &$option->add("multiupdate");
        $item->Body = "<a class=\"ew-action ew-multi-update\" title=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" data-table=\"employee_asset\" data-caption=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.femployee_assetlist,url:'" . GetUrl($this->MultiUpdateUrl) . "'});return false;\">" . $Language->phrase("UpdateSelectedLink") . "</a>";
        $item->Visible = $Security->canEdit();
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"femployee_assetlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"femployee_assetlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.femployee_assetlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // asset_name
        if (!$this->isAddOrEdit() && $this->asset_name->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->asset_name->AdvancedSearch->SearchValue != "" || $this->asset_name->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // serial_number
        if (!$this->isAddOrEdit() && $this->serial_number->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->serial_number->AdvancedSearch->SearchValue != "" || $this->serial_number->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // asset_received
        if (!$this->isAddOrEdit() && $this->asset_received->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->asset_received->AdvancedSearch->SearchValue != "" || $this->asset_received->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // asset_return
        if (!$this->isAddOrEdit() && $this->asset_return->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->asset_return->AdvancedSearch->SearchValue != "" || $this->asset_return->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.femployee_assetlist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.femployee_assetlist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.femployee_assetlist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_employee_asset" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_employee_asset\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.femployee_assetlist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"femployee_assetlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"employeeassetsearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"employee_asset\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SearchBtn',url:'employeeassetsearch'});\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
