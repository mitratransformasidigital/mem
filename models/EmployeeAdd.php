...
class EmployeeAdd extends Employee
{
    ...
    protected function validateForm()
    {
        ...
        if (in_array("activity", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PermitGrid");
        if (in_array("permit", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("EmployeeContractGrid");
        if (in_array("employee_contract", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("EmployeeAssetGrid");
        if (in_array("employee_asset", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("EmployeeTimesheetGrid");
        if (in_array("employee_timesheet", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
