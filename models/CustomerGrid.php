...
class CustomerGrid extends Customer
{
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->customer_id->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_customer_name") && $CurrentForm->hasValue("o_customer_name") && $this->customer_name->CurrentValue != $this->customer_name->OldValue) {
            return false;
        }
        ...
        if ($CurrentForm->hasValue("x_contact") && $CurrentForm->hasValue("o_contact") && $this->contact->CurrentValue != $this->contact->OldValue) {
            return false;
        }
        return true;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_customer_name")) {
            $this->customer_name->setOldValue($CurrentForm->getValue("o_customer_name"));
        }
        ...
        if ($CurrentForm->hasValue("o_contact")) {
            $this->contact->setOldValue($CurrentForm->getValue("o_contact"));
        }
        ...
        if (!$this->customer_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
    ...
    protected function editRow()
    {
        ...
        if ($this->customer_name->CurrentValue != "") { // Check field with unique index
            $filterChk = "(`customer_name` = '" . AdjustSql($this->customer_name->CurrentValue, $this->Dbid) . "')";
        ...
        if (!$rsold) {
            ...
            // contact
            $this->contact->setDbValueDef($rsnew, $this->contact->CurrentValue, "", $this->contact->ReadOnly);
