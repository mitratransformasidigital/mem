 ...
 * Page class
 */
class PrintQuotationSummary extends PrintQuotation
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'print_quotation';

    // Page object name
    public $PageObjName = "PrintQuotationSummary";
    ...
    public function __construct()
    {
        ...
        // Table object (print_quotation)
        if (!isset($GLOBALS["print_quotation"]) || get_class($GLOBALS["print_quotation"]) == PROJECT_NAMESPACE . "print_quotation") {
            $GLOBALS["print_quotation"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'print_quotation');
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_SEARCH) {
            ...
            if ($this->RowTotalType == ROWTOTAL_GROUP && $this->RowGroupLevel >= 2) {
                $this->RowAttrs["data-group-2"] = $this->quotation_date->groupValue(); // Set up group attribute 2
            }
            ...
            // quotation_date
            $this->quotation_date->GroupViewValue = $this->quotation_date->groupValue();
            $this->quotation_date->CellCssClass = ($this->RowGroupLevel == 2 ? "ew-rpt-grp-summary-2" : "ew-rpt-grp-field-2");
            $this->quotation_date->ViewCustomAttributes = "";
            $this->quotation_date->GroupViewValue = DisplayGroupValue($this->quotation_date, $this->quotation_date->GroupViewValue);
            ...
            // quotation_date
            $this->quotation_date->GroupViewValue = $this->quotation_date->groupValue();
            $this->quotation_date->CellCssClass = "ew-rpt-grp-field-2";
            $this->quotation_date->ViewCustomAttributes = "";
            $this->quotation_date->GroupViewValue = DisplayGroupValue($this->quotation_date, $this->quotation_date->GroupViewValue);
            if (!$this->quotation_date->LevelBreak) {
                $this->quotation_date->GroupViewValue = "&nbsp;";
            } else {
                $this->quotation_date->LevelBreak = false;
            }
        ...
        if ($this->RowType == ROWTYPE_TOTAL) {
            ...
            // quotation_date
            $currentValue = $this->quotation_date->GroupViewValue;
            $viewValue = &$this->quotation_date->GroupViewValue;
            ...
            // quotation_date
            $currentValue = $this->quotation_date->groupValue();
            $viewValue = &$this->quotation_date->GroupViewValue;
            $viewAttrs = &$this->quotation_date->ViewAttrs;
            $cellAttrs = &$this->quotation_date->CellAttrs;
            $hrefValue = &$this->quotation_date->HrefValue;
