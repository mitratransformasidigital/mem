 ...
 * Page class
 */
class UserlevelsUpdate extends Userlevels
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "UserlevelsUpdate";
    ...
    public function run()
    {
        ...
        if (count($this->RecKeys) <= 0) {
            $this->terminate("userlevelslist"); // No records selected, return to list
            return;
        }
        if ($this->isUpdate()) {
        ...
        if ($rs = $this->loadRecordset()) {
            ...
            while (!$rs->EOF) {
                if ($i == 1) {
                    ...
                    if (!CompareValue($this->userlevelname->DbValue, $rs->fields['userlevelname'])) {
    ...
    protected function loadFormValues()
    {
        ...
        $this->userlevelname->MultiUpdate = $CurrentForm->getValue("u_userlevelname");
    }
    ...
    protected function validateForm()
    {
        ...
        if ($this->userlevelname->multiUpdateSelected()) {
            $updateCnt++;
        }
        if ($updateCnt == 0) {
            return false;
        ...
        if ($this->userlevelname->Required) {
            if ($this->userlevelname->MultiUpdate != "" && !$this->userlevelname->IsDetailKey && EmptyValue($this->userlevelname->FormValue)) {
