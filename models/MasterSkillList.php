 ...
 * Page class
 */
class MasterSkillList extends MasterSkill
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'master_skill';

    // Page object name
    public $PageObjName = "MasterSkillList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fmaster_skilllist";
    ...
    public function __construct()
    {
        ...
        // Table object (master_skill)
        if (!isset($GLOBALS["master_skill"]) || get_class($GLOBALS["master_skill"]) == PROJECT_NAMESPACE . "master_skill") {
            $GLOBALS["master_skill"] = &$this;
        }
        ...
        $this->AddUrl = "masterskilladd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "masterskilldelete";
        $this->MultiUpdateUrl = "masterskillupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'master_skill');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fmaster_skilllistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("master_skill"));
                $doc->Text = @$content;
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->skill->AdvancedSearch->toJson(), ","); // Field skill
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmaster_skilllistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field skill
        $this->skill->AdvancedSearch->SearchValue = @$filter["x_skill"];
        $this->skill->AdvancedSearch->SearchOperator = @$filter["z_skill"];
        $this->skill->AdvancedSearch->SearchCondition = @$filter["v_skill"];
        $this->skill->AdvancedSearch->SearchValue2 = @$filter["y_skill"];
        $this->skill->AdvancedSearch->SearchOperator2 = @$filter["w_skill"];
        $this->skill->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->skill, $default, false); // skill
        ...
        if (!$default && $this->Command == "search") {
            $this->skill->AdvancedSearch->save(); // skill
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->skill, $arKeywords, $type);
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->skill->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
                $this->skill->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        // Restore advanced search values
                $this->skill->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->skill); // skill
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'myprofile')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("myprofile", "TblCaption");
            $body .= "&nbsp;" . str_replace("%c", $this->myprofile_Count, $Language->phrase("DetailCount"));
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("myprofilelist?" . Config("TABLE_SHOW_MASTER") . "=master_skill&" . GetForeignKeyUrl("fk_skill_id", $this->skill_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("MyprofileGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'master_skill')) {
                ...
                $detailViewTblVar .= "myprofile";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'master_skill')) {
                ...
                $detailEditTblVar .= "myprofile";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'master_skill')) {
        ...
        if ($Security->allowList(CurrentProjectID() . 'employee')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("employeelist?" . Config("TABLE_SHOW_MASTER") . "=master_skill&" . GetForeignKeyUrl("fk_skill_id", $this->skill_id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->skill_id->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                ...
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'master_skill') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
        ...
        $item->Body = "<a class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.fmaster_skilllist, url:'" . GetUrl($this->MultiDeleteUrl) . "', data:{action:'show'}});return false;\">" . $Language->phrase("DeleteSelectedLink") . "</a>";
        $item->Visible = $Security->canDelete();

        // Add multi update
        $item = &$option->add("multiupdate");
        $item->Body = "<a class=\"ew-action ew-multi-update\" title=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" data-table=\"master_skill\" data-caption=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.fmaster_skilllist,url:'" . GetUrl($this->MultiUpdateUrl) . "'});return false;\">" . $Language->phrase("UpdateSelectedLink") . "</a>";
        $item->Visible = $Security->canEdit();
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmaster_skilllistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmaster_skilllistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fmaster_skilllist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function renderListOptionsExt()
    {
        ...
        $links = "";
        $btngrps = "";
        $sqlwrk = "`skill_id`=" . AdjustSql($this->skill_id->CurrentValue, $this->Dbid) . "";
        ...
        if ($this->DetailPages && $this->DetailPages["myprofile"] && $this->DetailPages["myprofile"]->Visible) {
            ...
            $url = "myprofilepreview?t=master_skill&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"myprofile\" data-url=\"" . $url . "\">";
            if ($Security->allowList(CurrentProjectID() . 'master_skill')) {
                $label = $Language->TablePhrase("myprofile", "TblCaption");
                $label .= "&nbsp;" . JsEncode(str_replace("%c", $this->myprofile_Count, $Language->phrase("DetailCount")));
                $link = "<li class=\"nav-item\"><a href=\"#\" class=\"nav-link\" data-toggle=\"tab\" data-table=\"myprofile\" data-url=\"" . $url . "\">" . $label . "</a></li>";
                $links .= $link;
                $detaillnk = JsEncodeAttribute("myprofilelist?" . Config("TABLE_SHOW_MASTER") . "=master_skill&" . GetForeignKeyUrl("fk_skill_id", $this->skill_id->CurrentValue) . "");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'master_skill')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=myprofile");
                $btngrp .= "<a href=\"#\" class=\"mr-2\" title=\"" . HtmlTitle($caption) . "\" onclick=\"window.location='" . HtmlEncode($url) . "';return false;\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'master_skill')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=myprofile");
                $btngrp .= "<a href=\"#\" class=\"mr-2\" title=\"" . HtmlTitle($caption) . "\" onclick=\"window.location='" . HtmlEncode($url) . "';return false;\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'master_skill')) {
                $caption = $Language->phrase("MasterDetailCopyLink");
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=myprofile");
                $btngrp .= "<a href=\"#\" class=\"mr-2\" title=\"" . HtmlTitle($caption) . "\" onclick=\"window.location='" . HtmlEncode($url) . "';return false;\">" . $caption . "</a>";
            }
        ...
        if ($this->DetailPages && $this->DetailPages["employee"] && $this->DetailPages["employee"]->Visible) {
            ...
            $url = "employeepreview?t=master_skill&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"employee\" data-url=\"" . $url . "\">";
            if ($Security->allowList(CurrentProjectID() . 'master_skill')) {
                $label = $Language->TablePhrase("employee", "TblCaption");
                $label .= "&nbsp;" . JsEncode(str_replace("%c", $this->employee_Count, $Language->phrase("DetailCount")));
                $link = "<li class=\"nav-item\"><a href=\"#\" class=\"nav-link\" data-toggle=\"tab\" data-table=\"employee\" data-url=\"" . $url . "\">" . $label . "</a></li>";
                $links .= $link;
                $detaillnk = JsEncodeAttribute("employeelist?" . Config("TABLE_SHOW_MASTER") . "=master_skill&" . GetForeignKeyUrl("fk_skill_id", $this->skill_id->CurrentValue) . "");
    ...
    protected function loadSearchValues()
    {
        ...
        // skill
        if (!$this->isAddOrEdit() && $this->skill->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->skill->AdvancedSearch->SearchValue != "" || $this->skill->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->skill->setDbValue($row['skill']);
        $this->description->setDbValue($row['description']);
        $detailTbl = Container("myprofile");
        $detailFilter = $detailTbl->sqlDetailFilter_master_skill();
        $detailFilter = str_replace("@skill_id@", AdjustSql($this->skill_id->DbValue, "DB"), $detailFilter);
        $detailTbl->setCurrentMasterTable("master_skill");
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            // skill
            $this->skill->ViewValue = $this->skill->CurrentValue;
            $this->skill->ViewCustomAttributes = "";
            ...
            // skill
            $this->skill->LinkCustomAttributes = "";
            $this->skill->HrefValue = "";
            $this->skill->TooltipValue = "";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fmaster_skilllist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fmaster_skilllist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fmaster_skilllist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_master_skill" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_master_skill\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fmaster_skilllist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fmaster_skilllistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"masterskillsearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"master_skill\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SearchBtn',url:'masterskillsearch'});\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
