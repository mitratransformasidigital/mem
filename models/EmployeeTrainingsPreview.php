 ...
 * Page class
 */
class EmployeeTrainingsPreview extends EmployeeTrainings
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeeTrainingsPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "employee") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=employee&" . GetForeignKeyUrl("fk_employee_username", $this->employee_username->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeysFromFilter($f)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "employee") {
            $find = "`employee_username`=";
            $x = strpos($f, $find);
            if ($x !== false) {
                $x += strlen($find);
                $val = substr($f, $x);
                $val = $this->unquoteValue($val, "DB");
                 $this->employee_username->setQueryStringValue($val);
            }
        }
    }
