 ...
 * Page class
 */
class EmployeeShiftEdit extends EmployeeShift
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeeShiftEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("es_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->es_id->setQueryStringValue($keyValue);
                $this->es_id->setOldValue($this->es_id->QueryStringValue);
            } elseif (Post("es_id") !== null) {
                $this->es_id->setFormValue(Post("es_id"));
                $this->es_id->setOldValue($this->es_id->FormValue);
            } else {
            ...
            if ($this->isShow()) {
                ...
                if ($this->TotalRecords <= 0) { // No record found
                    ...
                    if ($this->es_id->CurrentValue != null) {
                        while (!$rs->EOF) {
                            if (SameString($this->es_id->CurrentValue, $rs->fields['es_id'])) {
    ...
    protected function validateForm()
    {
        ...
        if ($this->es_id->Required) {
            if (!$this->es_id->IsDetailKey && EmptyValue($this->es_id->FormValue)) {
                $this->es_id->addErrorMessage(str_replace("%s", $this->es_id->caption(), $this->es_id->RequiredErrorMessage));
            }
        }
