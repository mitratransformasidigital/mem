 ...
 * Page class
 */
class MasterHolidayList extends MasterHoliday
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MasterHolidayList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fmaster_holidaylist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "masterholidayadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "masterholidaydelete";
        $this->MultiUpdateUrl = "masterholidayupdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fmaster_holidaylistsrch";
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "master_shift") {
            $masterTbl = Container("master_shift");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("mastershiftlist"); // Return to master page
                return;
            } else {
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->holiday_id->AdvancedSearch->toJson(), ","); // Field holiday_id
        $filterList = Concat($filterList, $this->shift_id->AdvancedSearch->toJson(), ","); // Field shift_id
        $filterList = Concat($filterList, $this->holiday_date->AdvancedSearch->toJson(), ","); // Field holiday_date
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmaster_holidaylistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field holiday_id
        $this->holiday_id->AdvancedSearch->SearchValue = @$filter["x_holiday_id"];
        $this->holiday_id->AdvancedSearch->SearchOperator = @$filter["z_holiday_id"];
        $this->holiday_id->AdvancedSearch->SearchCondition = @$filter["v_holiday_id"];
        $this->holiday_id->AdvancedSearch->SearchValue2 = @$filter["y_holiday_id"];
        $this->holiday_id->AdvancedSearch->SearchOperator2 = @$filter["w_holiday_id"];
        $this->holiday_id->AdvancedSearch->save();

        // Field shift_id
        $this->shift_id->AdvancedSearch->SearchValue = @$filter["x_shift_id"];
        $this->shift_id->AdvancedSearch->SearchOperator = @$filter["z_shift_id"];
        $this->shift_id->AdvancedSearch->SearchCondition = @$filter["v_shift_id"];
        $this->shift_id->AdvancedSearch->SearchValue2 = @$filter["y_shift_id"];
        $this->shift_id->AdvancedSearch->SearchOperator2 = @$filter["w_shift_id"];
        $this->shift_id->AdvancedSearch->save();

        // Field holiday_date
        $this->holiday_date->AdvancedSearch->SearchValue = @$filter["x_holiday_date"];
        $this->holiday_date->AdvancedSearch->SearchOperator = @$filter["z_holiday_date"];
        $this->holiday_date->AdvancedSearch->SearchCondition = @$filter["v_holiday_date"];
        $this->holiday_date->AdvancedSearch->SearchValue2 = @$filter["y_holiday_date"];
        $this->holiday_date->AdvancedSearch->SearchOperator2 = @$filter["w_holiday_date"];
        $this->holiday_date->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->holiday_id, $default, false); // holiday_id
        $this->buildSearchSql($where, $this->shift_id, $default, false); // shift_id
        $this->buildSearchSql($where, $this->holiday_date, $default, false); // holiday_date
        $this->buildSearchSql($where, $this->holiday_name, $default, false); // holiday_name
        ...
        if (!$default && $this->Command == "search") {
            $this->holiday_id->AdvancedSearch->save(); // holiday_id
            $this->shift_id->AdvancedSearch->save(); // shift_id
            $this->holiday_date->AdvancedSearch->save(); // holiday_date
            $this->holiday_name->AdvancedSearch->save(); // holiday_name
        }
        return $where;
    }
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->holiday_name, $arKeywords, $type);
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->holiday_id->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->shift_id->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->holiday_date->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->holiday_name->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    {
                $this->holiday_id->AdvancedSearch->unsetSession();
                $this->shift_id->AdvancedSearch->unsetSession();
                $this->holiday_date->AdvancedSearch->unsetSession();
                $this->holiday_name->AdvancedSearch->unsetSession();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->shift_id); // shift_id
            $this->updateSort($this->holiday_date); // holiday_date
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->holiday_id->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.fmaster_holidaylist, url:'" . GetUrl($this->MultiDeleteUrl) . "', data:{action:'show'}});return false;\">" . $Language->phrase("DeleteSelectedLink") . "</a>";
        $item->Visible = $Security->canDelete();

        // Add multi update
        $item = &$option->add("multiupdate");
        $item->Body = "<a class=\"ew-action ew-multi-update\" title=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" data-table=\"master_holiday\" data-caption=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.fmaster_holidaylist,url:'" . GetUrl($this->MultiUpdateUrl) . "'});return false;\">" . $Language->phrase("UpdateSelectedLink") . "</a>";
        $item->Visible = $Security->canEdit();
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmaster_holidaylistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmaster_holidaylistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fmaster_holidaylist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // holiday_id
        if (!$this->isAddOrEdit() && $this->holiday_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->holiday_id->AdvancedSearch->SearchValue != "" || $this->holiday_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // shift_id
        if (!$this->isAddOrEdit() && $this->shift_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->shift_id->AdvancedSearch->SearchValue != "" || $this->shift_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // holiday_date
        if (!$this->isAddOrEdit() && $this->holiday_date->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->holiday_date->AdvancedSearch->SearchValue != "" || $this->holiday_date->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // holiday_name
        if (!$this->isAddOrEdit() && $this->holiday_name->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->holiday_name->AdvancedSearch->SearchValue != "" || $this->holiday_name->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->holiday_id->setDbValue($row['holiday_id']);
        $this->shift_id->setDbValue($row['shift_id']);
        $this->holiday_date->setDbValue($row['holiday_date']);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $curVal = trim(strval($this->shift_id->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->shift_id->AdvancedSearch->ViewValue = $this->shift_id->lookupCacheOption($curVal);
            } else {
                $this->shift_id->AdvancedSearch->ViewValue = $this->shift_id->Lookup !== null && is_array($this->shift_id->Lookup->Options) ? $curVal : null;
            }
            if ($this->shift_id->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->shift_id->EditValue = array_values($this->shift_id->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`shift_id`" . SearchString("=", $this->shift_id->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fmaster_holidaylist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fmaster_holidaylist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fmaster_holidaylist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_master_holiday" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_master_holiday\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fmaster_holidaylist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fmaster_holidaylistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"masterholidaysearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"master_holiday\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SearchBtn',url:'masterholidaysearch'});\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "master_shift") {
            $master_shift = Container("master_shift");
            $rsmaster = $master_shift->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $master_shift;
                    $master_shift->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
