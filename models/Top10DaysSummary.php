 ...
 * Page class
 */
class Top10DaysSummary extends Top10Days
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'Top 10 Days';

    // Page object name
    public $PageObjName = "Top10DaysSummary";
    ...
    public function __construct()
    {
        ...
        // Table object (Top_10_Days)
        if (!isset($GLOBALS["Top_10_Days"]) || get_class($GLOBALS["Top_10_Days"]) == PROJECT_NAMESPACE . "Top_10_Days") {
            $GLOBALS["Top_10_Days"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Top 10 Days');
        }
    ...
    {
        $data = [];
        $data["timesheet_id"] = $record['timesheet_id'];
        $data["employee_username"] = $record['employee_username'];
        $data["year"] = $record['year'];
        $data["month"] = $record['month'];
        $data["days"] = $record['days'];
        $data["sick"] = $record['sick'];
        $data["leave"] = $record['leave'];
        $data["permit"] = $record['permit'];
        $data["absence"] = $record['absence'];
        $data["timesheet_doc"] = $record['timesheet_doc'];
        $data["approved"] = $record['approved'];
        $this->Rows[] = $data;
        $this->timesheet_id->setDbValue($record['timesheet_id']);
        $this->employee_username->setDbValue($record['employee_username']);
        $this->year->setDbValue($record['year']);
        $this->month->setDbValue($record['month']);
        $this->days->setDbValue($record['days']);
        $this->sick->setDbValue($record['sick']);
        $this->leave->setDbValue($record['leave']);
        $this->permit->setDbValue($record['permit']);
        $this->absence->setDbValue($record['absence']);
        $this->timesheet_doc->Upload->DbValue = $record['timesheet_doc'];
        $this->employee_notes->setDbValue($record['employee_notes']);
        $this->company_notes->setDbValue($record['company_notes']);
        $this->approved->setDbValue($record['approved']);
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_TOTAL && $this->RowTotalSubType == ROWTOTAL_FOOTER && $this->RowTotalType == ROWTOTAL_PAGE) { // Get Page total
            ...
            $this->employee_username->getCnt($records);
            $this->days->getSum($records);
            ...
            if ($rsagg) {
                $this->employee_username->Count = $this->TotalCount;
                $this->employee_username->CntValue = $rsagg["cnt_employee_username"];
                $this->year->Count = $this->TotalCount;
                $this->month->Count = $this->TotalCount;
                $this->days->Count = $this->TotalCount;
                $this->days->SumValue = $rsagg["sum_days"];
                $hasSummary = true;
            }
            ...
            if (!$hasCount || !$hasSummary) {
                ...
                $this->employee_username->getCnt($this->DetailRecords);
                $this->days->getSum($this->DetailRecords);
            }
        }
        ...
        if ($this->RowType == ROWTYPE_SEARCH) {
            ...
            // employee_username
            $this->employee_username->CntViewValue = $this->employee_username->CntValue;
            $curVal = strval($this->employee_username->CntValue);
            if ($curVal != "") {
                $this->employee_username->CntViewValue = $this->employee_username->lookupCacheOption($curVal);
                if ($this->employee_username->CntViewValue === null) { // Lookup from database
                    ...
                    if ($ari > 0) { // Lookup values found
                        ...
                        $this->employee_username->CntViewValue = $this->employee_username->displayValue($arwrk);
                    } else {
                        $this->employee_username->CntViewValue = $this->employee_username->CntValue;
                    }
                }
            } else {
                $this->employee_username->CntViewValue = null;
            }
            $this->employee_username->ViewCustomAttributes = "";
            $this->employee_username->CellAttrs["class"] = ($this->RowTotalType == ROWTOTAL_PAGE || $this->RowTotalType == ROWTOTAL_GRAND) ? "ew-rpt-grp-aggregate" : "ew-rpt-grp-summary-" . $this->RowGroupLevel;

            // days
            $this->days->SumViewValue = $this->days->SumValue;
            $this->days->SumViewValue = FormatNumber($this->days->SumViewValue, 0, -2, -2, -2);
            $this->days->CellCssStyle .= "text-align: right;";
            $this->days->ViewCustomAttributes = "";
            $this->days->CellAttrs["class"] = ($this->RowTotalType == ROWTOTAL_PAGE || $this->RowTotalType == ROWTOTAL_GRAND) ? "ew-rpt-grp-aggregate" : "ew-rpt-grp-summary-" . $this->RowGroupLevel;
            ...
            $this->employee_username->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "ew-table-row");
            $this->employee_username->ViewCustomAttributes = "";
            ...
            $this->days->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "ew-table-row");
            $this->days->CellCssStyle .= "text-align: right;";
            $this->days->ViewCustomAttributes = "";
        ...
        if ($this->RowType == ROWTYPE_TOTAL) {
            // employee_username
            $currentValue = $this->employee_username->CntValue;
            $viewValue = &$this->employee_username->CntViewValue;
            $viewAttrs = &$this->employee_username->ViewAttrs;
            $cellAttrs = &$this->employee_username->CellAttrs;
            $hrefValue = &$this->employee_username->HrefValue;
            $linkAttrs = &$this->employee_username->LinkAttrs;
            $this->cellRendered($this->employee_username, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);

            // days
            $currentValue = $this->days->SumValue;
            $viewValue = &$this->days->SumViewValue;
            $viewAttrs = &$this->days->ViewAttrs;
            $cellAttrs = &$this->days->CellAttrs;
            $hrefValue = &$this->days->HrefValue;
            $linkAttrs = &$this->days->LinkAttrs;
            $this->cellRendered($this->days, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
        } else {
            // employee_username
            $currentValue = $this->employee_username->CurrentValue;
            $viewValue = &$this->employee_username->ViewValue;
            $viewAttrs = &$this->employee_username->ViewAttrs;
            $cellAttrs = &$this->employee_username->CellAttrs;
            $hrefValue = &$this->employee_username->HrefValue;
            $linkAttrs = &$this->employee_username->LinkAttrs;
            ...
            // days
            $currentValue = $this->days->CurrentValue;
            $viewValue = &$this->days->ViewValue;
            $viewAttrs = &$this->days->ViewAttrs;
            $cellAttrs = &$this->days->CellAttrs;
            $hrefValue = &$this->days->HrefValue;
            $linkAttrs = &$this->days->LinkAttrs;
    ...
    protected function setupFieldCount()
    {
        ...
        if ($this->employee_username->Visible) {
    ...
    {
        if ($this->DrillDown) {
            return "`year` DESC,`month` DESC,`employee_username` ASC";
        }
        ...
        if ($this->getOrderBy() == "") {
            ...
            if ($useDefaultSort) {
                ...
                $this->setOrderBy("`year` DESC,`month` DESC,`employee_username` ASC");
            }
        }
