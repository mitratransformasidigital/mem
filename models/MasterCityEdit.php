 ...
 * Page class
 */
class MasterCityEdit extends MasterCity
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MasterCityEdit";
    ...
    public function run()
    {
            ...
            if ($this->isShow()) {
                ...
                if ($this->TotalRecords <= 0) { // No record found
                    ...
                    if ($this->city_id->CurrentValue != null) {
                        while (!$rs->EOF) {
                            if (SameString($this->city_id->CurrentValue, $rs->fields['city_id'])) {
        ...
        switch ($this->CurrentAction) {
            ...
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "mastercitylist") {
    ...
    protected function editRow()
    {
        ...
        $conn = $this->getConnection();
        if ($this->city->CurrentValue != "") { // Check field with unique index
            $filterChk = "(`city` = '" . AdjustSql($this->city->CurrentValue, $this->Dbid) . "')";
            ...
            if ($rsChk->fetch()) {
                $idxErrMsg = str_replace("%f", $this->city->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->city->CurrentValue, $idxErrMsg);
        ...
        if (!$rsold) {
            ...
            // Check referential integrity for master table 'master_province'
            $validMasterRecord = true;
            $masterFilter = $this->sqlMasterFilter_master_province();
            $keyValue = $rsnew['province_id'] ?? $rsold['province_id'];
            if (strval($keyValue) != "") {
                $masterFilter = str_replace("@province_id@", AdjustSql($keyValue), $masterFilter);
            } else {
                $validMasterRecord = false;
            }
            if ($validMasterRecord) {
                $rsmaster = Container("master_province")->loadRs($masterFilter)->fetch();
                $validMasterRecord = $rsmaster !== false;
            }
            if (!$validMasterRecord) {
                $relatedRecordMsg = str_replace("%t", "master_province", $Language->phrase("RelatedRecordRequired"));
            ...
            if ($updateRow) {
                ...
                if ($editRow) {
                    $detailPage = Container("MasterOfficeGrid");
                    if (in_array("master_office", $detailTblVar) && $detailPage->DetailEdit) {
                        $Security->loadCurrentUserLevel($this->ProjectID . "master_office"); // Load user level of detail table
