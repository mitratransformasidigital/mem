...
/**
 * Table class for permit
 */
class Permit extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'permit';
        $this->TableName = 'permit';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`permit`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // permit_id
        $this->permit_id = new DbField('permit', 'permit', 'x_permit_id', 'permit_id', '`permit_id`', '`permit_id`', 3, 11, -1, false, '`permit_id`', false, false, false, 'FORMATTED TEXT', 'HIDDEN');
        $this->permit_id->IsAutoIncrement = true; // Autoincrement field
        $this->permit_id->IsPrimaryKey = true; // Primary key field
        $this->permit_id->Sortable = false; // Allow sort
        $this->permit_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['permit_id'] = &$this->permit_id;

        // employee_username
        $this->employee_username = new DbField('permit', 'permit', 'x_employee_username', 'employee_username', '`employee_username`', '`employee_username`', 200, 100, -1, false, '`employee_username`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        $this->permit_type->Nullable = false; // NOT NULL field
        $this->permit_type->Required = true; // Required field
        $this->permit_type->Sortable = true; // Allow sort
        ...
        // document
        $this->document = new DbField('permit', 'permit', 'x_document', 'document', '`document`', '`document`', 200, 250, -1, true, '`document`', false, false, false, 'FORMATTED TEXT', 'FILE');
        $this->document->Sortable = true; // Allow sort
        $this->Fields['document'] = &$this->document;

        // note
        $this->note = new DbField('permit', 'permit', 'x_note', 'note', '`note`', '`note`', 201, 500, -1, false, '`note`', false, false, false, 'FORMATTED TEXT', 'TEXTAREA');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`permit`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->permit_id->setDbValue($conn->lastInsertId());
            $rs['permit_id'] = $this->permit_id->DbValue;
        }
        return $success;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("permitlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "permitedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "permitadd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("permitview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("permitview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "permitadd?" . $this->getUrlParm($parm);
        } else {
            $url = "permitadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("permitedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("permitadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("permitdelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function getFileData($fldparm, $key, $resize, $width = 0, $height = 0, $plugins = [])
    {
        ...
        if (count($ar) == 1) {
            $this->permit_id->CurrentValue = $ar[0];
        } else {
