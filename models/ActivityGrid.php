 ...
 * Page class
 */
class ActivityGrid extends Activity
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ActivityGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "factivitygrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->activity_id->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_activity_date") && $CurrentForm->hasValue("o_activity_date") && $this->activity_date->CurrentValue != $this->activity_date->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_time_in") && $CurrentForm->hasValue("o_time_in") && $this->time_in->CurrentValue != $this->time_in->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_time_out") && $CurrentForm->hasValue("o_time_out") && $this->time_out->CurrentValue != $this->time_out->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x__action") && $CurrentForm->hasValue("o__action") && $this->_action->CurrentValue != $this->_action->OldValue) {
            return false;
        }
        if (!EmptyValue($this->document->Upload->Value)) {
            return false;
        }
    ...
    public function resetFormError()
    {
        ...
        $this->activity_date->clearErrorMessage();
        $this->time_in->clearErrorMessage();
        $this->time_out->clearErrorMessage();
        $this->_action->clearErrorMessage();
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->activity_date->CurrentValue = date("Y/m/d");
        $this->activity_date->OldValue = $this->activity_date->CurrentValue;
        $this->time_in->CurrentValue = date("H:i");
        $this->time_in->OldValue = $this->time_in->CurrentValue;
        $this->time_out->CurrentValue = null;
        $this->time_out->OldValue = $this->time_out->CurrentValue;
        $this->_action->CurrentValue = null;
        $this->_action->OldValue = $this->_action->CurrentValue;
        $this->document->Upload->DbValue = null;
        $this->document->OldValue = $this->document->Upload->DbValue;
        $this->document->Upload->Index = $this->RowIndex;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_activity_date")) {
            $this->activity_date->setOldValue($CurrentForm->getValue("o_activity_date"));
        }
        ...
        if ($CurrentForm->hasValue("o_time_in")) {
            $this->time_in->setOldValue($CurrentForm->getValue("o_time_in"));
        }
        ...
        if ($CurrentForm->hasValue("o_time_out")) {
            $this->time_out->setOldValue($CurrentForm->getValue("o_time_out"));
        }
        ...
        if (!$this->activity_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
    ...
    protected function newRow()
    {
        ...
        $row['activity_date'] = $this->activity_date->CurrentValue;
        $row['time_in'] = $this->time_in->CurrentValue;
        $row['time_out'] = $this->time_out->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (is_numeric($this->RowIndex)) {
                RenderUploadField($this->document, $this->RowIndex);
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // activity_date
        $this->activity_date->setDbValueDef($rsnew, UnFormatDateTime($this->activity_date->CurrentValue, 5), null, false);

        // time_in
        $this->time_in->setDbValueDef($rsnew, $this->time_in->CurrentValue, null, false);

        // time_out
        $this->time_out->setDbValueDef($rsnew, $this->time_out->CurrentValue, null, false);
        ...
        // document
        if ($this->document->Visible && !$this->document->Upload->KeepFile) {
            $this->document->Upload->DbValue = ""; // No need to delete old file
            if ($this->document->Upload->FileName == "") {
                $rsnew['document'] = null;
            } else {
                $rsnew['document'] = $this->document->Upload->FileName;
        ...
        if ($this->document->Visible && !$this->document->Upload->KeepFile) {
            ...
            if (!EmptyValue($this->document->Upload->FileName)) {
                ...
                $this->document->Upload->DbValue = empty($oldFiles) ? "" : implode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $oldFiles);
                $this->document->Upload->FileName = implode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $newFiles);
                $this->document->setDbValueDef($rsnew, $this->document->Upload->FileName, null, false);
            }
        }
