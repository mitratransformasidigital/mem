 ...
 * Page class
 */
class EmployeeContractList extends EmployeeContract
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeeContractList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "femployee_contractlist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "employeecontractdelete";
        $this->MultiUpdateUrl = "employeecontractupdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option femployee_contractlistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->thr->AdvancedSearch->toJson(), ","); // Field thr
        $filterList = Concat($filterList, $this->contract_start->AdvancedSearch->toJson(), ","); // Field contract_start
        $filterList = Concat($filterList, $this->contract_end->AdvancedSearch->toJson(), ","); // Field contract_end
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "femployee_contractlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field thr
        $this->thr->AdvancedSearch->SearchValue = @$filter["x_thr"];
        $this->thr->AdvancedSearch->SearchOperator = @$filter["z_thr"];
        $this->thr->AdvancedSearch->SearchCondition = @$filter["v_thr"];
        $this->thr->AdvancedSearch->SearchValue2 = @$filter["y_thr"];
        $this->thr->AdvancedSearch->SearchOperator2 = @$filter["w_thr"];
        $this->thr->AdvancedSearch->save();

        // Field contract_start
        $this->contract_start->AdvancedSearch->SearchValue = @$filter["x_contract_start"];
        $this->contract_start->AdvancedSearch->SearchOperator = @$filter["z_contract_start"];
        $this->contract_start->AdvancedSearch->SearchCondition = @$filter["v_contract_start"];
        $this->contract_start->AdvancedSearch->SearchValue2 = @$filter["y_contract_start"];
        $this->contract_start->AdvancedSearch->SearchOperator2 = @$filter["w_contract_start"];
        $this->contract_start->AdvancedSearch->save();

        // Field contract_end
        $this->contract_end->AdvancedSearch->SearchValue = @$filter["x_contract_end"];
        $this->contract_end->AdvancedSearch->SearchOperator = @$filter["z_contract_end"];
        $this->contract_end->AdvancedSearch->SearchCondition = @$filter["v_contract_end"];
        $this->contract_end->AdvancedSearch->SearchValue2 = @$filter["y_contract_end"];
        $this->contract_end->AdvancedSearch->SearchOperator2 = @$filter["w_contract_end"];
        $this->contract_end->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->bonus, $default, false); // bonus
        $this->buildSearchSql($where, $this->thr, $default, false); // thr
        $this->buildSearchSql($where, $this->contract_start, $default, false); // contract_start
        $this->buildSearchSql($where, $this->contract_end, $default, false); // contract_end
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->bonus->AdvancedSearch->save(); // bonus
            $this->thr->AdvancedSearch->save(); // thr
            $this->contract_start->AdvancedSearch->save(); // contract_start
            $this->contract_end->AdvancedSearch->save(); // contract_end
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->contract_document, $arKeywords, $type);
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->bonus->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->thr->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->contract_start->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->contract_end->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
                ...
                $this->bonus->AdvancedSearch->unsetSession();
                $this->thr->AdvancedSearch->unsetSession();
                $this->contract_start->AdvancedSearch->unsetSession();
                $this->contract_end->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        // Restore advanced search values
                ...
                $this->bonus->AdvancedSearch->load();
                $this->thr->AdvancedSearch->load();
                $this->contract_start->AdvancedSearch->load();
                $this->contract_end->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->thr); // thr
            $this->updateSort($this->contract_start); // contract_start
            $this->updateSort($this->contract_end); // contract_end
            $this->updateSort($this->office_id); // office_id
            $this->updateSort($this->contract_document); // contract_document
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->bonus->setSort("");
                $this->thr->setSort("");
                $this->contract_start->setSort("");
                $this->contract_end->setSort("");
                $this->office_id->setSort("");
                $this->contract_document->setSort("");
                $this->notes->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->contract_id->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.femployee_contractlist, url:'" . GetUrl($this->MultiDeleteUrl) . "', data:{action:'show'}});return false;\">" . $Language->phrase("DeleteSelectedLink") . "</a>";
        $item->Visible = $Security->canDelete();

        // Add multi update
        $item = &$option->add("multiupdate");
        $item->Body = "<a class=\"ew-action ew-multi-update\" title=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" data-table=\"employee_contract\" data-caption=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.femployee_contractlist,url:'" . GetUrl($this->MultiUpdateUrl) . "'});return false;\">" . $Language->phrase("UpdateSelectedLink") . "</a>";
        $item->Visible = $Security->canEdit();
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"femployee_contractlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"femployee_contractlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.femployee_contractlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // salary
        if (!$this->isAddOrEdit() && $this->salary->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // bonus
        if (!$this->isAddOrEdit() && $this->bonus->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->bonus->AdvancedSearch->SearchValue != "" || $this->bonus->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // thr
        if (!$this->isAddOrEdit() && $this->thr->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->thr->AdvancedSearch->SearchValue != "" || $this->thr->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        if (is_array($this->thr->AdvancedSearch->SearchValue)) {
            $this->thr->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->thr->AdvancedSearch->SearchValue);
        }
        if (is_array($this->thr->AdvancedSearch->SearchValue2)) {
            $this->thr->AdvancedSearch->SearchValue2 = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->thr->AdvancedSearch->SearchValue2);
        }

        // contract_start
        if (!$this->isAddOrEdit() && $this->contract_start->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->contract_start->AdvancedSearch->SearchValue != "" || $this->contract_start->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // contract_end
        if (!$this->isAddOrEdit() && $this->contract_end->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->contract_end->AdvancedSearch->SearchValue != "" || $this->contract_end->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.femployee_contractlist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.femployee_contractlist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.femployee_contractlist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_employee_contract" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_employee_contract\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.femployee_contractlist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"femployee_contractlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"employeecontractsearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"employee_contract\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SearchBtn',url:'employeecontractsearch'});\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
