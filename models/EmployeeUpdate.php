 ...
 * Page class
 */
class EmployeeUpdate extends Employee
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeeUpdate";
    ...
    public function run()
    {
        ...
        if (count($this->RecKeys) <= 0) {
            $this->terminate("employeelist"); // No records selected, return to list
            return;
        }
        if ($this->isUpdate()) {
        ...
        if ($rs = $this->loadRecordset()) {
            ...
            while (!$rs->EOF) {
                if ($i == 1) {
                    ...
                    if (!CompareValue($this->user_level->DbValue, $rs->fields['user_level'])) {
    ...
    protected function validateForm()
    {
        ...
        if ($this->user_level->Required) {
            if ($this->user_level->MultiUpdate != "" && !$this->user_level->IsDetailKey && EmptyValue($this->user_level->FormValue)) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // user_level
            if ($Security->canAdmin()) { // System admin
                $this->user_level->setDbValueDef($rsnew, $this->user_level->CurrentValue, null, $this->user_level->ReadOnly || $this->user_level->MultiUpdate != "1");
            }
