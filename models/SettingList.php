...
class SettingList extends Setting
{
    ...
    public function __construct()
    {
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fsettinglistsrch";
    ...
    public function run()
    {
        ...
        $this->setting_value->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->setting_id->AdvancedSearch->toJson(), ","); // Field setting_id
        $filterList = Concat($filterList, $this->setting_name->AdvancedSearch->toJson(), ","); // Field setting_name
        $filterList = Concat($filterList, $this->setting_value->AdvancedSearch->toJson(), ","); // Field setting_value
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function restoreFilterList()
    {
        ...
        // Field setting_name
        $this->setting_name->AdvancedSearch->SearchValue = @$filter["x_setting_name"];
        $this->setting_name->AdvancedSearch->SearchOperator = @$filter["z_setting_name"];
        $this->setting_name->AdvancedSearch->SearchCondition = @$filter["v_setting_name"];
        $this->setting_name->AdvancedSearch->SearchValue2 = @$filter["y_setting_name"];
        $this->setting_name->AdvancedSearch->SearchOperator2 = @$filter["w_setting_name"];
        $this->setting_name->AdvancedSearch->save();

        // Field setting_value
        $this->setting_value->AdvancedSearch->SearchValue = @$filter["x_setting_value"];
        $this->setting_value->AdvancedSearch->SearchOperator = @$filter["z_setting_value"];
        $this->setting_value->AdvancedSearch->SearchCondition = @$filter["v_setting_value"];
        $this->setting_value->AdvancedSearch->SearchValue2 = @$filter["y_setting_value"];
        $this->setting_value->AdvancedSearch->SearchOperator2 = @$filter["w_setting_value"];
        $this->setting_value->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->setting_id, $default, false); // setting_id
        $this->buildSearchSql($where, $this->setting_name, $default, false); // setting_name
        $this->buildSearchSql($where, $this->setting_value, $default, false); // setting_value
        ...
        if (!$default && $this->Command == "search") {
            $this->setting_id->AdvancedSearch->save(); // setting_id
            $this->setting_name->AdvancedSearch->save(); // setting_name
            $this->setting_value->AdvancedSearch->save(); // setting_value
        }
        return $where;
    }
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->setting_name, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->setting_value, $arKeywords, $type);
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->setting_id->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->setting_name->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->setting_value->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    {
                $this->setting_id->AdvancedSearch->unsetSession();
                $this->setting_name->AdvancedSearch->unsetSession();
                $this->setting_value->AdvancedSearch->unsetSession();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->setting_name); // setting_name
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->setting_name->setSort("");
                $this->setting_value->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->setting_id->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.fsettinglist, url:'" . GetUrl($this->MultiDeleteUrl) . "', data:{action:'show'}});return false;\">" . $Language->phrase("DeleteSelectedLink") . "</a>";
        $item->Visible = $Security->canDelete();

        // Add multi update
        $item = &$option->add("multiupdate");
        $item->Body = "<a class=\"ew-action ew-multi-update\" title=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" data-table=\"setting\" data-caption=\"" . HtmlTitle($Language->phrase("UpdateSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.fsettinglist,url:'" . GetUrl($this->MultiUpdateUrl) . "'});return false;\">" . $Language->phrase("UpdateSelectedLink") . "</a>";
        $item->Visible = $Security->canEdit();
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsettinglistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsettinglistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fsettinglist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // setting_id
        if (!$this->isAddOrEdit() && $this->setting_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->setting_id->AdvancedSearch->SearchValue != "" || $this->setting_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // setting_name
        if (!$this->isAddOrEdit() && $this->setting_name->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->setting_name->AdvancedSearch->SearchValue != "" || $this->setting_name->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // setting_value
        if (!$this->isAddOrEdit() && $this->setting_value->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->setting_value->AdvancedSearch->SearchValue != "" || $this->setting_value->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->setting_id->setDbValue($row['setting_id']);
        $this->setting_name->setDbValue($row['setting_name']);
        $this->setting_value->setDbValue($row['setting_value']);
    }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fsettinglist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fsettinglist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fsettinglist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fsettinglistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"settingsearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        $item->Visible = true;
