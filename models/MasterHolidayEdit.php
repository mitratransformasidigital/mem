 ...
 * Page class
 */
class MasterHolidayEdit extends MasterHoliday
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MasterHolidayEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("holiday_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->holiday_id->setQueryStringValue($keyValue);
                $this->holiday_id->setOldValue($this->holiday_id->QueryStringValue);
            } elseif (Post("holiday_id") !== null) {
                $this->holiday_id->setFormValue(Post("holiday_id"));
                $this->holiday_id->setOldValue($this->holiday_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("holiday_id") ?? Route("holiday_id")) !== null) {
                    $this->holiday_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
            ...
            if ($this->isShow()) {
                ...
                if ($this->TotalRecords <= 0) { // No record found
                    ...
                    $this->terminate("masterholidaylist"); // Return to list page
                    return;
                    ...
                    if ($this->holiday_id->CurrentValue != null) {
                        while (!$rs->EOF) {
                            if (SameString($this->holiday_id->CurrentValue, $rs->fields['holiday_id'])) {
        ...
        switch ($this->CurrentAction) {
            ...
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "masterholidaylist") {
    ...
    protected function validateForm()
    {
        ...
        if ($this->holiday_id->Required) {
            if (!$this->holiday_id->IsDetailKey && EmptyValue($this->holiday_id->FormValue)) {
                $this->holiday_id->addErrorMessage(str_replace("%s", $this->holiday_id->caption(), $this->holiday_id->RequiredErrorMessage));
            }
        }
