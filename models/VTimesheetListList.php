 ...
 * Page class
 */
class VTimesheetListList extends VTimesheetList
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'v_timesheet_list';

    // Page object name
    public $PageObjName = "VTimesheetListList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fv_timesheet_listlist";
    ...
    public function __construct()
    {
        ...
        // Table object (v_timesheet_list)
        if (!isset($GLOBALS["v_timesheet_list"]) || get_class($GLOBALS["v_timesheet_list"]) == PROJECT_NAMESPACE . "v_timesheet_list") {
            $GLOBALS["v_timesheet_list"] = &$this;
        }
        ...
        $this->AddUrl = "vtimesheetlistadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "vtimesheetlistdelete";
        $this->MultiUpdateUrl = "vtimesheetlistupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'v_timesheet_list');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fv_timesheet_listlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("v_timesheet_list"));
                $doc->Text = @$content;
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->work_date->AdvancedSearch->toJson(), ","); // Field work_date
        $filterList = Concat($filterList, $this->time_in->AdvancedSearch->toJson(), ","); // Field time_in
        $filterList = Concat($filterList, $this->time_out->AdvancedSearch->toJson(), ","); // Field time_out
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fv_timesheet_listlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field work_date
        $this->work_date->AdvancedSearch->SearchValue = @$filter["x_work_date"];
        $this->work_date->AdvancedSearch->SearchOperator = @$filter["z_work_date"];
        $this->work_date->AdvancedSearch->SearchCondition = @$filter["v_work_date"];
        $this->work_date->AdvancedSearch->SearchValue2 = @$filter["y_work_date"];
        $this->work_date->AdvancedSearch->SearchOperator2 = @$filter["w_work_date"];
        $this->work_date->AdvancedSearch->save();

        // Field time_in
        $this->time_in->AdvancedSearch->SearchValue = @$filter["x_time_in"];
        $this->time_in->AdvancedSearch->SearchOperator = @$filter["z_time_in"];
        $this->time_in->AdvancedSearch->SearchCondition = @$filter["v_time_in"];
        $this->time_in->AdvancedSearch->SearchValue2 = @$filter["y_time_in"];
        $this->time_in->AdvancedSearch->SearchOperator2 = @$filter["w_time_in"];
        $this->time_in->AdvancedSearch->save();

        // Field time_out
        $this->time_out->AdvancedSearch->SearchValue = @$filter["x_time_out"];
        $this->time_out->AdvancedSearch->SearchOperator = @$filter["z_time_out"];
        $this->time_out->AdvancedSearch->SearchCondition = @$filter["v_time_out"];
        $this->time_out->AdvancedSearch->SearchValue2 = @$filter["y_time_out"];
        $this->time_out->AdvancedSearch->SearchOperator2 = @$filter["w_time_out"];
        $this->time_out->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->work_date, $default, false); // work_date
        $this->buildSearchSql($where, $this->time_in, $default, false); // time_in
        $this->buildSearchSql($where, $this->time_out, $default, false); // time_out
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->work_date->AdvancedSearch->save(); // work_date
            $this->time_in->AdvancedSearch->save(); // time_in
            $this->time_out->AdvancedSearch->save(); // time_out
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->work_date->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->time_in->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->time_out->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
                $this->employee_username->AdvancedSearch->loadDefault();
                $this->work_date->AdvancedSearch->loadDefault();
        return true;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
                ...
                $this->work_date->AdvancedSearch->unsetSession();
                $this->time_in->AdvancedSearch->unsetSession();
                $this->time_out->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        // Restore advanced search values
                ...
                $this->work_date->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->work_date); // work_date
            $this->updateSort($this->time_in); // time_in
            $this->updateSort($this->time_out); // time_out
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`work_date` ASC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->work_date->getSort() != "") {
                    $useDefaultSort = false;
                }
                if ($useDefaultSort) {
                    $this->work_date->setSort("ASC");
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->work_date->setSort("");
                $this->time_in->setSort("");
                $this->time_out->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->work_date->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fv_timesheet_listlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fv_timesheet_listlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fv_timesheet_listlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // work_date
        if (!$this->isAddOrEdit() && $this->work_date->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->work_date->AdvancedSearch->SearchValue != "" || $this->work_date->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // time_in
        if (!$this->isAddOrEdit() && $this->time_in->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->time_in->AdvancedSearch->SearchValue != "" || $this->time_in->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // time_out
        if (!$this->isAddOrEdit() && $this->time_out->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->time_out->AdvancedSearch->SearchValue != "" || $this->time_out->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->work_date->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->work_date->AdvancedSearch->SearchValue, 0), 8));
            $this->work_date->PlaceHolder = RemoveHtml($this->work_date->caption());
            $this->work_date->EditAttrs["class"] = "form-control";
            $this->work_date->EditCustomAttributes = "";
            $this->work_date->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->work_date->AdvancedSearch->SearchValue2, 0), 8));
            $this->work_date->PlaceHolder = RemoveHtml($this->work_date->caption());
            ...
            $this->days->PlaceHolder = RemoveHtml($this->days->caption());
                ...
                $this->work_date->Count = 0; // Initialize count
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckDate($this->work_date->AdvancedSearch->SearchValue)) {
            $this->work_date->addErrorMessage($this->work_date->getErrorMessage(false));
        }
        if (!CheckDate($this->work_date->AdvancedSearch->SearchValue2)) {
            $this->work_date->addErrorMessage($this->work_date->getErrorMessage(false));
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fv_timesheet_listlist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fv_timesheet_listlist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fv_timesheet_listlist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_v_timesheet_list" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_v_timesheet_list\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fv_timesheet_listlist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fv_timesheet_listlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"vtimesheetlistsearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"v_timesheet_list\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SearchBtn',url:'vtimesheetlistsearch'});\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
