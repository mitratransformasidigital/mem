 ...
 * Page class
 */
class MasterProvinceUpdate extends MasterProvince
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MasterProvinceUpdate";
    ...
    public function run()
    {
        ...
        $this->province_id->Required = false;
        ...
        if (count($this->RecKeys) <= 0) {
            $this->terminate("masterprovincelist"); // No records selected, return to list
            return;
        }
        if ($this->isUpdate()) {
        ...
        if ($rs = $this->loadRecordset()) {
            ...
            while (!$rs->EOF) {
                if ($i == 1) {
                    ...
                    if (!CompareValue($this->province_id->DbValue, $rs->fields['province_id'])) {
                        $this->province_id->CurrentValue = null;
                    }
                    if (!CompareValue($this->province->DbValue, $rs->fields['province'])) {
    ...
    {
        $keyFld = $key;
        $this->province_id->OldValue = $keyFld;
        return true;
    }
    ...
    protected function loadFormValues()
    {
        ...
        $this->province_id->MultiUpdate = $CurrentForm->getValue("u_province_id");
        ...
        $this->province->MultiUpdate = $CurrentForm->getValue("u_province");
    }
    ...
    protected function validateForm()
    {
        global $Language;
        $updateCnt = 0;
        if ($this->province_id->multiUpdateSelected()) {
            $updateCnt++;
        }
        if ($this->province->multiUpdateSelected()) {
            $updateCnt++;
        }
        if ($updateCnt == 0) {
            return false;
        ...
        if ($this->province_id->Required) {
            if ($this->province_id->MultiUpdate != "" && !$this->province_id->IsDetailKey && EmptyValue($this->province_id->FormValue)) {
        ...
        if ($this->province->Required) {
            if ($this->province->MultiUpdate != "" && !$this->province->IsDetailKey && EmptyValue($this->province->FormValue)) {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // province_id
            $this->province_id->setDbValueDef($rsnew, $this->province_id->CurrentValue, "", $this->province_id->ReadOnly || $this->province_id->MultiUpdate != "1");

            // province
            $this->province->setDbValueDef($rsnew, $this->province->CurrentValue, "", $this->province->ReadOnly || $this->province->MultiUpdate != "1");
